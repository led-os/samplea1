...
import com.go.gl.view.GLContentView.SurfaceViewOwner;
import com.go.gl.view.GLLayoutInflater;
import com.go.gl.view.GLView;

/**
 * 
 * <br>类描述: 引擎的 Main Activity
 * <br>功能详细描述:
 * 
 * <p> 几个重要的方法：
 * 	<ul>
 ...
 * @date  [2012-9-5]
 */
public class GLActivity extends Activity implements OnGlobalLayoutListener, SurfaceViewOwner {
	 ...
	 * @param smoothFullScreenTransition	是否需要平滑的全屏切换过程，如果是，则窗口和屏幕一样大，那么View在非全屏的时候排版需要考虑通知栏的高度
	 * @param getStatusBarHeight			是否需要获取状态栏的高度
	 */
	protected void onCreate(Bundle savedInstanceState, boolean smoothFullScreenTransition, boolean getStatusBarHeight) {
	...
	/**
	 * <br>功能简述: 设置在整个屏幕内排版
	 * <br>功能详细描述: 为了避免在全屏与非全屏状态之间切换要改变surface的大小造成卡顿，可以统一大小为屏幕大小
	 * <br>注意:
	 */
	private void setLayoutInScreen() {
		...
		if (mGLSurfaceView != null) {
			mGLSurfaceView.setTranslateY(!mFullScreen && mLayoutInScreen
	...
	/**
	 * <br>功能简述: 设置OpenGL内容的根容器
	 * <br>功能详细描述:
	 * <br>注意:
	 * 用户内容除了OpenGL内容外，还可以包含其他的2D内容，例如菜单，根据实际需要传入<var>setContent</var>参数。
	 * 
	 * <p>如果在{@link #onCreate(Bundle, boolean, boolean)}时指定要获取状态栏高度，那么会将<var>view</var>
	 * 设置为{@link View#GONE}，等到延迟获取到时({@link #initDefaultStatusBarHeight(int)})再恢复。</p>
	 * 
	 * @param view			OpenGL场景的根容器
	 * @param setContent	是否调用{@link Activity#setContentView(View)}，让<var>view</var>作为用户内容的根容器
	 * 
	...
	public void setSurfaceView(GLContentView view, boolean setContent) {
		...
		if (DBG) {
			GLContentView.sLog.dns(TAG, "setSurfaceView view=" + view + " content=" + setContent + " this=" + this);
		}
		...
		if (setContent) {
			...
			if (viewGroup != null) {
				super.addContentView(viewGroup, new LayoutParams(LayoutParams.FILL_PARENT,
						LayoutParams.FILL_PARENT));
			}
		}
		mGLSurfaceView = view;
		mGLSurfaceView.setTranslateY(mFullScreen && !mLayoutInScreen ? 0 : mStatusBarStaticHeight);
	 ...
	 * <br>注意:
	 * @return
	 * @deprecated 使用名称更符合意义的{@link #getSurfaceView()}代替，同时也避免和{@link #getContentGlView()}混淆
	 */
     ...
     * 在onPause里面会往Handler加入这个runnable
     * 在onResume里面会移除这个runnable
     */
...
//			GLContentView.sLog.ins(TAG, "onResume " + this);
	...
	protected void setFullScreen(boolean fullScreen) {
		...
		if (fullScreen) {
			...
			if (mGLSurfaceView != null) {
				mGLSurfaceView.setTranslateY(!mLayoutInScreen ? 0 : mStatusBarStaticHeight);
			}
		}
	...
	/**
	 * <br>功能简述: 当这个Activity创建后会去获取状态栏的默认高度，这个方法是当获取到时的回调方法
	 * <br>功能详细描述:
	 * <br>注意: 获取和回调之间可能是异步的
	 * @param height	状态栏的默认高度
	 */
	...
	/**
	 * <br>功能简述: 获取状态栏当前显示的高度
	 * <br>功能详细描述:
	 * <br>注意: 如果使用{@link #setFullScreen(boolean)}设置了全屏，那么返回值是0。
	 * @return 状态栏当前显示的高度
	 * @see {@link #getStatusBarStaticHeight()}
	 */
	...
	public void onGlobalLayout() {
		...
		if (DBG) {
			GLContentView.sLog.dns(TAG, "onGlobalLayout time=" + sOnGlobalLayoutTime 
					+ " this=" + this + " view=" + mGLSurfaceView);
		}
		...
		mGetStatusBarHeightAction = new Runnable() {
			public void run() {
				...
				if (DBG) {
					GLContentView.sLog.vns(TAG, "getWindowVisibleDisplayFrame " + frame 
							+ " activity=" + GLActivity.this + " view=" + mGLSurfaceView);
				}
				// 如果是在其他全屏应用下进入桌面，则需要等待状态栏恢复再重试
				if (frame.width() == sDefaultDisplayWidth 
						&& frame.height() == sDefaultDisplayHeight) {
					if (System.currentTimeMillis() - sOnGlobalLayoutTime > 500) {	//CHEKCSTYLE IGNORE
						...
						if ((attrs.flags & WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN) == WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN) {
							...
							frame.top = Math.round(getResources().getDisplayMetrics().density * DEFAULT_STATUS_BAR_HEIGHT_IN_DP);
						}
					} else {
				...
				if (DBG) {
					GLContentView.sLog.dns(TAG, "initDefaultStatusBarHeight " + frame.top 
							+ " activity=" + GLActivity.this + " view=" + mGLSurfaceView);
				}
				initDefaultStatusBarHeight(frame.top);
			}
	 ...
	 * <br>功能详细描述:
	 * <br>注意:
	 * @param setContent 将新创建的GLContentview设为ContentView
	 */
	protected final void reCreateSurfaceView(boolean setContent) {
		...
		GLContentView newContentView = new GLContentView(getApplicationContext(), oldGLContentView.isTranslucent());
	...
	/**
	 * {@inheritDoc}
	 * 如果没有特殊情况（例如外部保持GLContentView的引用，或者将它加载到其他视图容器），可以调用{@link #reCreateSurfaceView(boolean)}这个默认实现。
	 * 如果重载了就不要调用super的本方法，避免重复。
	 */
	@Override
...
/**
 * 
 * <br>类描述: 全局排版事件监听者的封装者
 * <br>功能详细描述:
 * 如果需要获取系统状态栏高度，那么需要注册全局排版时间监听。为了避免被系统保存了引用造成内存泄露，采用封装的方式。
 * 
 * @author  dengweiming
 * @date  [2012-11-5]
 */
