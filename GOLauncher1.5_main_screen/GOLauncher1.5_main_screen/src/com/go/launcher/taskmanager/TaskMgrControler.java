...
public class TaskMgrControler extends Controler implements ICleanable {
	...
	public ArrayList<FunTaskItemInfo> getProgresses() {
		...
		ArrayList<_APPINFOR> tasks = (ArrayList<_APPINFOR>) getTaskManager()
				.retriveAppList().clone();
		if (null == tasks) {
			return null;
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ArrayList<ProgressesInfo> getProgressesInfoList() {
		ArrayList<_APPINFOR> tasks = (ArrayList<_APPINFOR>) getTaskManager()
				.retriveAppList().clone();
		ArrayList<ProgressesInfo> infoList = new ArrayList<ProgressesInfo>();
		if (null == tasks) {
			return null;
		}
		_APPINFOR info = null;
		...
		for (int i = 0; i < size; ++i) {
			info = tasks.get(i);
			if (null == info || null == info._infor) {
				continue;
			}
			intent = info._infor;
			ProgressesInfo singleInfo = new ProgressesInfo();
			singleInfo.mIntent = intent;
			singleInfo.mPid = info._pid;
			singleInfo.mIsInWhiteList = isIgnoreTask(intent);
	...
	public void terminateAll(ArrayList<FunTaskItemInfo> infos) {
		long memOld = getTaskManager().retriveAvailableMemory() / 1024;
		terminateTaskItemInfos(infos);
		// 通知
		broadCast(TERMINATE_ALL, 0, null, null);

		long memNew = getTaskManager().retriveAvailableMemory() / 1024;
	...
	private void initTaskHashMap() {
		...
		if (funTaskAdditionalInfos == null
				|| funTaskAdditionalInfos.size() <= 0) {
			return;
		}
	...
	public ArrayList<AppItemInfo> getAllAppItemInfos() {
		...
		// 首先从白名单中获取程序信息
		Iterator<Entry<ComponentName, FunTaskAdditionalInfo>> it = mTaskHashMap
				.entrySet().iterator();
		while (it.hasNext()) {
		...
		Iterator<Entry<ComponentName, AppItemInfo>> iter = allAppsMap
				.entrySet().iterator();
		while (iter.hasNext()) {
		...
		SortHelper.doSort(itemInfos.subList(0, middle), method);
		SortHelper.doSort(itemInfos.subList(middle, itemInfos.size()), method);
