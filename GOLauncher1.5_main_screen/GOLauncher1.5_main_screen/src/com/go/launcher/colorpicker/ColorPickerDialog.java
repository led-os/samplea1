...
public class ColorPickerDialog extends BaseDialog implements View.OnClickListener {
	...
	private PrivatePreference mPreferencesManager = null;
	...
	private int mDefaultColor = convertToColorInt(DEFAULT_COLOR);
	/**
	 * 
	 * <br>
	 * 类描述: <br>
	...
	private class ColorPickerView extends View {
		...
		ColorPickerView(Context c, OnColorChangedListener l, int color) {
			...
			DeskThemeControler themeControler = core.getDeskThemeControler();
			...
			if (themeBean != null && themeBean.mScreen != null) {
				if (null != themeBean.mScreen.mFont) {
					if (themeBean.mScreen.mFont.mColor != 0) {
						mDefaultColor = themeBean.mScreen.mFont.mColor;
					}
				}
			}
		}
		...
		protected void onDraw(Canvas canvas) {
			...
			if (mFlag) {
				...
				if (mHuanBitmap == null) {
					mHuanBitmap = BitmapFactory.decodeResource(getResources(),
							R.drawable.desk_setting_huan);
				}
				int w = mHuanBitmap.getWidth() / 2;
				int h = mHuanBitmap.getHeight() / 2;

				canvas.drawBitmap(mHuanBitmap, x - w, y - h, null);
			}
	...
	public ColorPickerDialog(Context context, OnColorChangedListener listener, boolean custom,
		...
		mPreferencesManager = PrivatePreference.getPreference(mContext);
		if (x != NOXY && y != NOXY) {
			mPointX = x;
			mPointY = y;
		} else {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		LinearLayout dialogLayout = (LinearLayout) mView.findViewById(R.id.dialog_layout);
		setDialogWidth(dialogLayout, mContext);
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
				LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT);
		LinearLayout colorLayout = (LinearLayout) mView.findViewById(R.id.color_layout);
		...
		mAlphaText = (TextView) mView.findViewById(R.id.desk_setting_colorpicker_seekbar_title);
		TextView minTextView = (TextView) mView.findViewById(R.id.min_value);
		TextView maxTextView = (TextView) mView.findViewById(R.id.max_value);

		mTransparencyBar = (SeekBar) mView.findViewById(R.id.desk_setting_colorpicker_seekbar);
		...
		mEditText = (EditText) mView.findViewById(R.id.color_edit_text);
		mEditText.addTextChangedListener(mEditTextListener);
		if (mCustom) {
			mColorText = convertToARGB(mInitialColor);
			mEditText.setText(mColorText);
		} else {
			mColorText = convertToARGB(mDefaultColor);
			mEditText.setText(mColorText);
		}
		setPositiveButton(R.string.change_icon_reset_default, this);
		setNegativeButton(R.string.cancel, this);

		setOtherButtonVisible(View.VISIBLE);
		setOtherButton(R.string.ok, this);
		if (mType == FONT) {
	...
	public void setTitleVisible() {
		if (GoLauncherActivityProxy.getOrientation() == Configuration.ORIENTATION_LANDSCAPE) {
...
//	}
	...
	private void saveXY(float pointX, float ponitY) {
		if (mType == FONT) {
			mPreferencesManager.putFloat(PrefConst.KEY_FONT_INITIA_X, pointX);
			mPreferencesManager.putFloat(PrefConst.KEY_FONT_INITIA_Y, ponitY);
			mPreferencesManager.putString(PrefConst.KEY_FONT_COLOR, mColor);

		} else {
			mPreferencesManager.putFloat(PrefConst.KEY_ICON_INITIA_X, pointX);
			mPreferencesManager.putFloat(PrefConst.KEY_ICON_INITIA_Y, ponitY);
			mPreferencesManager.putString(PrefConst.KEY_ICON_COLOR, mColor);
		}
	...
	private void saveXYToDefault() {
		if (mType == FONT) {
			mPreferencesManager.putFloat(PrefConst.KEY_FONT_INITIA_X, -18);
			mPreferencesManager.putFloat(PrefConst.KEY_FONT_INITIA_Y, -122);
			mPreferencesManager.putString(PrefConst.KEY_FONT_COLOR, DEFAULT_COLOR);
		} else {
			mPreferencesManager.putFloat(PrefConst.KEY_ICON_INITIA_X, -18);
			mPreferencesManager.putFloat(PrefConst.KEY_ICON_INITIA_Y, -122);
			mPreferencesManager.putString(PrefConst.KEY_ICON_COLOR, DEFAULT_COLOR);
		}
		mPreferencesManager.commit();
	}
	...
	private void getXY() {
		if (mType == FONT) {
			mPointX = mPreferencesManager.getFloat(PrefConst.KEY_FONT_INITIA_X, -18);
			mPointY = mPreferencesManager.getFloat(PrefConst.KEY_FONT_INITIA_Y, -122);
			mColor = mPreferencesManager.getString(PrefConst.KEY_FONT_COLOR, DEFAULT_COLOR);
		} else {
			mPointX = mPreferencesManager.getFloat(PrefConst.KEY_ICON_INITIA_X, -18);
			mPointY = mPreferencesManager.getFloat(PrefConst.KEY_ICON_INITIA_Y, -122);
			mColor = mPreferencesManager.getString(PrefConst.KEY_ICON_COLOR, DEFAULT_COLOR);
		}
	...
	public void onClick(View source) {
		if (source == mOtherButtonLayout) {
			...
			dismiss();
		} else if (source == mCancelButtonLayout) {
			dismiss();
		} else if (source == mOkButtonLayout) {		
	...
	public View getContentView() {
		mView = getLayoutInflater().inflate(R.layout.desk_setting_color_select_view, null);
		return mView;
	}
