...
public class MediaManagementOpenChooser {
	...
	private MediaManagementOpenChooser(Context context) {
		...
		initViews();
		// 注册消息接收器，接受横竖屏切换事件
		MsgMgrProxy.registMsgHandler(new MsgHanlder());
	}
	...
	private void showDialog() {
		...
		if (mDialog == null) {
			mDialog = new Dialog(mContext, R.style.media_open_chooser_dialog){
				...
				public void dismiss() {
					super.dismiss();
					DialogStatusObserver.getInstance().onSimpleDialogDismiss(mDialog);
				}
			};
...
//					mInfo.album, isOpenBySearch, mInfo });
			...
			MsgMgrProxy.sendMessage(this, IDiyFrameIds.MEDIA_MANAGEMENT_FRAME,
					IMediaManagementMsgId.SWITCH_MEDIA_MANAGEMENT_CONTENT, -1, new Object[] { AppFuncContentTypes.MUSIC_PLAYER, (long) mInfo.dbId,
							mInfo.album, isOpenBySearch, mInfo }, null);
		}
	}
...
//				AppFuncConstants.OPEN_IMAGE_BROWSER, mObjects);
		...
		MsgMgrProxy.sendMessage(this, IDiyFrameIds.MEDIA_MANAGEMENT_FRAME,
				IMediaManagementMsgId.OPEN_IMAGE_BROWSER, -1, mObjects, null);
	}
	...
	public class MsgHanlder implements IMessageHandler {
		...
		public int getMsgHandlerId() {
			return IDiyFrameIds.MEDIAMANAGEMENTOPENCHOOSER_MSGHANLDER;
		}
