...
import com.jiubang.ggheart.apps.desks.appfunc.menu.ShellSwitchListMenu;
...
public class SwitchMenuControler implements ISwitchMenuControler {
	...
	private ShellSwitchListMenu mShellListMenu;
	...
	public SwitchMenuControler(Activity activity, View rootView) {
		mActivity = activity;
		mThemeController = AppFuncThemeController.getInstance(mActivity);
	...
	private void showMenu(int type, OnSwitchMenuItemClickListener listener) {
		if (type == AppFuncContentTypes.APP) {
			if (mShellListMenu == null) {
				mShellListMenu = new ShellSwitchListMenu(mActivity);
			}
			mShellListMenu.setOnSwitchMenuItemClickListener(listener);
			mShellListMenu.setItemResources(createMenuItemResouces(type));
			mShellListMenu.show(mRootView);
		} else {
