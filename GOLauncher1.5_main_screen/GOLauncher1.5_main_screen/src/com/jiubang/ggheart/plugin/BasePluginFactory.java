...
public abstract class BasePluginFactory {
	...
	protected static ClassLoader createClassLoaderFromDex(Context context, String pluginPackageName, String dexFileName) {
		ClassLoader loader = null;
		try {
			String dexPath = getDexPathFromApkCache(context, dexFileName);
			String dexOutputDir = getDexOutputDir(context, pluginPackageName);
			Log.i("BasePluginFactory", "-----------dexPath=" + dexPath + ", dexOutputDir=" + dexOutputDir);
			loader = new DexClassLoader(dexPath, dexOutputDir, null, context.getClassLoader());
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static String getDexPathFromApkCache(Context context, String dexFileName) throws NameNotFoundException {
		String dexPath = FileUtil.copyFileToApkCacheFromAsset(context, dexFileName);
		Log.i("BasePluginFactory", "============copyFileToApkCacheFromAsset=" + dexPath);
		return dexPath;
	}
	...
	public static void deleteDexFile(String pluginPackageName) {
		try {
			File file = new File(getDexOutputDir(ApplicationProxy.getContext(), pluginPackageName));
			if (!file.exists()) {
				return;
			}
			if (file.isDirectory()) {
