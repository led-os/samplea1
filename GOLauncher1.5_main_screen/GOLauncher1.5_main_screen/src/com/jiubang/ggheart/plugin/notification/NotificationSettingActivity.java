...
//	private static final int MORE_APPLICATION = 0;
	...
	private Button mNotificationok, mNotificationcancle;
...
//	private boolean mIsFirstStart;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mNotificationok = (Button) findViewById(R.id.notificationok);
		mNotificationcancle = (Button) findViewById(R.id.notificationcancle);
		mEnableButton = (ToggleButton) findViewById(R.id.enable_button);
		mListView = (ListView) findViewById(R.id.notificationlist);
		mNotificationok.setOnClickListener(this);
		mNotificationcancle.setOnClickListener(this);
		...
		boolean isNotificationEnable = pm.getBoolean(IPreferencesIds.PREFERENCE_IS_NOTIFICATION_PLUGIN_ENABLE, true);
		mEnableButton.setChecked(isNotificationEnable);
		mListView.setEnabled(isNotificationEnable);
		mIsEnable = isNotificationEnable;
		if (!Machine.IS_JELLY_BEAN_3) {
			mEnableButton.setVisibility(View.GONE); // 非4.3以上系统不显示
		}
	}
...
//				ShortCutSettingInfo.mAutoMissfacebookStatistic,
		...
		for (int i = 0; i < size; i++) {
			...
			if (item.getPackageName().equals("SMS")) { // 最上两项是电话和短信，固定属于已安装
				...
				if (GoAppUtils.isAppExist(this, PackageName.GMAIL)) {
					item.setInstalled(true);
				...
				if (AppUtils.isAppExist(this, item.getPackageName()) && !item.getPackageName().equals(PackageName.SINA_WEIBO)) {
					item.setInstalled(true);
					try {
...
//			convertView.setEnabled(mEnableButton.isChecked());
			return convertView;
		}
		...
		switch (id) {
			case R.id.notificationok :
				if (Machine.IS_JELLY_BEAN_3) {
					...
					PreferencesManager pm = new PreferencesManager(mContext);
					pm.putBoolean(
							IPreferencesIds.PREFERENCE_IS_NOTIFICATION_PLUGIN_ENABLE,
							mEnableButton.isChecked());
					pm.commit();
				} else {
					onOK();
				}
			...
			case R.id.enable_button :
				if (mEnableButton.isChecked()) {
					...
					DialogConfirm normalDialog = new DialogConfirm(this);
					normalDialog.show();
					normalDialog.setTitle(R.string.notification_setting_remove);
					normalDialog.setMessage(R.string.notification_setting_remove_tips);
					normalDialog.setPositiveButton(R.string.cancel, new OnClickListener() {
					...
					normalDialog.setNegativeButton(R.string.notification_setting_remove, new OnClickListener() {
						
						@Override
						public void onClick(View arg0) {
							NotificationInvoke.stopAllMonitor(getBaseContext());
							mListView.setEnabled(false);
							enableCheckBox(false);
						}
					});
					normalDialog.setOnCancelListener(new OnCancelListener() {
...
//			mIsFirstStart = true;
	...
	private void checkNeedShowTip() {
		...
		boolean needShowTip = pref.getBoolean(PrefConst.KEY_NEEDSHOWTIP, true);
		if (needShowTip) {
			if (VersionControl.getFirstRun()) {
				return;
			}
			if (!GoAppUtils.isAppExist(mContext, PackageName.FACEBOOK)) {
				return;
			}
			DeskToast.makeText(mContext, R.string.notification_setting_show_facebook_move,
					Toast.LENGTH_SHORT).show();
		}
		pref.putBoolean(PrefConst.KEY_NEEDSHOWTIP, false);
		pref.commit();
	}
	...
	private void commitResult(ArrayList<String> addItemList, ArrayList<Intent> addIntentList) {
		...
		SettingProxy.updateShortCutSettingNonIndepenceTheme(shortCutSettingInfo);
...
//		}
	...
	public boolean handleMessage(Object who, int msgId, int param, Object... objs) {
		switch (msgId) {
			case ICommonMsgId.SINA_WEIBO_LOGIN_FAIL :
				for (NotificationSettingItem item : mListItems) {
					if (item.getItemId() == NotificationType.NOTIFICATIONTYPE_SinaWeibo
							&& item.isInstalled) {
			...
			case ICommonMsgId.FACEBOOK_LOGIN_FAIL :
				for (NotificationSettingItem item : mListItems) {
