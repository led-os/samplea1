...
public class PinyinTool {
	...
	*/  
	...
	private static ChineseSpelling delegate = ChineseSpelling.getInstance();
	
    public static String converterToFirstSpell(String chs) {          
    	String key, value;
    	//String lastPinyin = "zuo";
		StringBuilder pinyinBuilder = new StringBuilder();
		for (int i = 0; i < chs.length(); i++) {
			key = chs.substring(i, i + 1);
			//int length = key.getBytes().length;
			if (key.getBytes().length == 3) {
				value = (String) delegate.convert(key);
				if (value == null) {
					value = key; //不能转则 不转
				} /*else if ( value.equals(lastPinyin) ) {	//可能没找到
					value = converterToSpell(chs.charAt(i));	//使用pinyin4j找
					if (value == null) {
						value = key; //不能转则 不转
					}
				}*/ //TODO 影响效率，暂时屏蔽
			...
			pinyinBuilder.append(value.charAt(0)); //取首字母
		}
...
//    private static String converterToSpell(char chineseChar){          
//        String pinyin = "";
//        HanyuPinyinOutputFormat defaultFormat = getPinyinOutputFormat();
//  
//       if ( chineseChar > 128) {   
//           try {   
//        	    String[] charAllPinyin = PinyinHelper.toHanyuPinyinStringArray(chineseChar, defaultFormat);
//        	    if( charAllPinyin == null 
//        	    		|| charAllPinyin.length == 0 
//        	    		|| charAllPinyin[0] == null ) {
//        	    	pinyin += chineseChar;   //没找到拼音，用本身
//        	    } else {
//        	    	pinyin += charAllPinyin[0];   
//        	    }
//            } catch (BadHanyuPinyinOutputFormatCombination e) {   
//            	pinyin += chineseChar;
//            } catch (java.lang.OutOfMemoryError e) { //内存不足处理
//                pinyin += chineseChar;
//                BgDataEx.add(BgDataEx.ERROR_OUT_OF_MEMMORY);
//            }
//
//        }else{   
//            pinyin += chineseChar;   //非汉字，不转
//        }   
//       return pinyin;   
//    }   
	
	/** 
	    * 汉字串转换位汉语拼音首字母的模式串(处理多音字)，英文字符不变 
	    * @param chines 汉字 
	    * @return 拼音 
	    */  
/*	public static String converterToFirstSpellPattern(String chines)
	{         
	    String pinyinName = "";  
	    ...
	    {  
	        ...
	        {  
	            try {        
					String[] charAllPinyin = PinyinHelper.toHanyuPinyinStringArray(nameChar[i], defaultFormat);
					StringBuilder charAllPinyinPattern = new StringBuilder();
	                
					int pinyinNum = charAllPinyin.length;
					if( pinyinNum == 1 ) //不是多音字
					{	 
						charAllPinyinPattern.append(charAllPinyin[0].charAt(0)); //首字母
					} else {
						charAllPinyinPattern.append("[");
						for(int j=0; j < pinyinNum-1; j++ )
						{
							 charAllPinyinPattern.append(charAllPinyin[j].charAt(0)); //首字母
							 charAllPinyinPattern.append("|");
						}
						charAllPinyinPattern.append(charAllPinyin[pinyinNum-1].charAt(0)); //首字母
						charAllPinyinPattern.append("]") ;
					}
					pinyinName += charAllPinyinPattern;
				} catch (BadHanyuPinyinOutputFormatCombination e) {  
				    e.printStackTrace();  
				}  
	        }else{  
	...
	*/
	 ...
	 * @return 拼音
	 */
	public static String converterToSpell(String chs) {
		String key, value;
		//String lastPinyin = "zuo";
		StringBuilder pinyinBuilder = new StringBuilder();
		for (int i = 0; i < chs.length(); i++) {
			...
			if (key.getBytes().length == 3) {
				 ...
				 * else if ( value.equals(lastPinyin) ) { //可能没找到 value =
				 * converterToSpell(chs.charAt(i)); //使用pinyin4j找 if (value ==
				 * null) { value = key; //不能转则 不转 } }
				 */// TODO 影响效率，暂时屏蔽
			} else {
				value = key;
			}

			pinyinBuilder.append(value);
		}
		return pinyinBuilder.toString();
	}
	 ...
	 * @return 拼音
	 */
	public static String converterToSpell(String chs,ContactDataItem data) {
		String key, value;
		//String lastPinyin = "zuo";
		StringBuilder pinyinBuilder = new StringBuilder();
		...
		for (int i = 0; i < chs.length(); i++) {
			...
			if (key.getBytes().length == 3) {
				...
				if (value == null) {
					value = key; // 不能转则 不转
					sortKeyBuilder.append(key);
				}else{
					sortKeyBuilder.append(value);
					sortKeyBuilder.append(key);
				}
				/*
				 * else if ( value.equals(lastPinyin) ) { //可能没找到 value =
				...
				value = key;
				sortKeyBuilder.append(key.toLowerCase());
			}
			
			pinyinBuilder.append(value);
		}
		
		data.setSortKey(sortKeyBuilder.toString());
		return pinyinBuilder.toString();
	}
	...
	//返回多音字拼音串的所有组合
	/**
	 * 如：区建军 返回List：[qujianjun,oujianjun]
	 */
/*	public static ArrayList<String> converterToSpellCombination(String chines)
	{         
    	ArrayList<String> pinyinNameComposeOld = new ArrayList<String>();
            ...
            {  
                try {         
					...
					if( pinyinNum == 1 ) //不是多音字
					{
						if( i==0 ) 
						{
							pinyinNameComposeOld.add(charAllPinyin[0]); //
						} else {
							for(int k=0; k < pinyinNameComposeOld.size(); k++)
							{
								String old = pinyinNameComposeOld.get(k); 
								old +=  charAllPinyin[0];
								pinyinNameComposeOld.set(k, old);
							}
						}
					} else {
						ArrayList<String> pinyinNameComposeNew = new ArrayList<String>();          		         
           		 		String oldStr = "";
	                	for(int j=0; j < pinyinNum; j++ )
	                	{   
	                		if( pinyinNameComposeOld.size() == 0 ) {
                				 pinyinNameComposeOld.add(""); //新建一个空串
                			}
	                		 
	                		String curStr = charAllPinyin[j];
	                		if ( !curStr.equals(oldStr) )  //不考虑拼音一样的
	                		{	                
	                			ArrayList<String> pinyinNameComposeCur =  (ArrayList<String>) pinyinNameComposeOld.clone();
	                       		for(int k=0; k < pinyinNameComposeCur.size(); k++)
	                       		{
	                       			String old = pinyinNameComposeOld.get(k); 
	                       			old +=  charAllPinyin[j];
	                       			pinyinNameComposeCur.set(k, old); //分叉
	                       		}    		 
	                       		pinyinNameComposeNew.addAll(pinyinNameComposeCur);
	                		}
	                		oldStr = curStr;		 
	                	}
	                	pinyinNameComposeOld = pinyinNameComposeNew;
	               	 }   
                } catch (BadHanyuPinyinOutputFormatCombination e) {  
                    e.printStackTrace();  
                }  
            }else{  
            	for(int j=0; j < pinyinNameComposeOld.size(); j++)
          		 {
          			 String old = pinyinNameComposeOld.get(j); 
          			 old +=  nameChar[i];
          			 pinyinNameComposeOld.set(j,old); 
          		 }
            } 
        }
        return pinyinNameComposeOld;  
    }  */
	
	private static class ChineseSpelling {
