...
public class MutilCheckGridView extends ViewGroup
	...
	public void initAdapterLayoutData() {
		...
		int perHeight = line_height;
		if (mCellHeight != -1) {
			perHeight = mCellHeight;
		}
		// 行数
		...
		realWidth = GoLauncherActivityProxy.getOrientation() == 1 ? width_v : width_h;
	...
	private void countTotalNum(int count) {
		...
		if (port) {
			...
			int iconSize = SettingProxy.getDesktopSettingInfo()
					.getIconRealSize(
							SettingProxy.getDesktopSettingInfo().getIconSizeStyle());
			int w = 0;
			if (mNewWidth != 0) {
				w = mNewWidth;
				mNewWidth = 0;
