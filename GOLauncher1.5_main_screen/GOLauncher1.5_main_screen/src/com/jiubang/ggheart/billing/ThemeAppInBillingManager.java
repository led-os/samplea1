...
public class ThemeAppInBillingManager {
	...
	public static final String EXTRA_FOR_COUPONID = "couponId";
	 ...
	 * 字段值无效用-1表示
	 */
	public static final int STATISTICS_DATA_DEFAULT_VALUE = -1;

	/**
	 * 谷歌市场应用内付费
	 */
	public static final int STATISTICS_DATA_PURCHASE_TYPE = 5;
	/**
	 * 优惠劵付费
	 */
	 ...
	 * 统计数据，未付费
	 */
	public static final int STATISTICS_DATA_NO_PAIED = 0;

	/**
	 * 统计数据，付费成功
	 */
	public static final int STATISTICS_DATA_HAS_PAIED = 1;

	/**
	 * 统计数据，上传的url
	 */
	public static String sStatisticsDataUrl = LauncherEnv.Server.STATISTICS_DATA_URL_PRO;
	static {
		if (ServerUtils.isUseTestServer(LauncherEnv.Server.STATISTICS_DATA_CONFIG_USE_TEST_SERVER)) {
			sStatisticsDataUrl = LauncherEnv.Server.STATISTICS_DATA_URL_SIT;
		}
	}
	...
	public void requestPurchase(String packageName, String productId,
		...
		sendBillingStatisticsData(packageName, STATISTICS_DATA_PURCHASE_TYPE,
				STATISTICS_DATA_NO_PAIED, "-1");
	}

	private void sendBillingStatisticsData(final String packageName, final int paidType,
			final int paidSucessed, final String couponId) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				try {
					String data = createData(packageName, paidType, paidSucessed, couponId);
					if (data != null) {
						String statisticsData = getStatisticsData(data);
						StringBuilder sb = new StringBuilder(sStatisticsDataUrl);
						sb.append(Machine.getAndroidId());
						sb.append("&goid=");
						sb.append(StatisticsManager.getGOID(mContext));
	...
	private String createData(String packageName, int paidType, int paidSucessed, String couponId) {
		if (mContext != null && packageName != null) {
			...
			stringBuffer.append(imei).append(STATISTICS_DATA_SEPARATE_STRING).append(country)
					...
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //获取金币方式
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //是否点击
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //tapjoy是否成功返回金币
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //是否安装桌面
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //是否激活
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //是否带桌面安装
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //价格
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //安装时间戳
					.append(STATISTICS_DATA_SEPARATE_STRING).append(STATISTICS_DATA_DEFAULT_VALUE) //付费成功时间戳
					.append(STATISTICS_DATA_SEPARATE_STRING).append(GoStorePhoneStateUtil.getUid(mContext)) //渠道号
					.append(STATISTICS_DATA_SEPARATE_STRING).append(couponId); //优惠券号
