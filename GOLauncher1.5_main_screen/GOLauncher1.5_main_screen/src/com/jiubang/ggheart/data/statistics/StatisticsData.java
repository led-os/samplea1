...
//	public final static int SEARCH_ID_GAMES = 2; // 搜索ID:游戏中心
	...
	public static final int DESK_ACTION_ID_GESTURE_PINCHOUT = 20;
	public static final int DESK_ACTION_ID_GESTURE_SWIPEUP = 21;
	public static final int DESK_ACTION_ID_GESTURE_ROTATECCW = 22;
	public static final int DESK_ACTION_ID_GESTURE_ROTATECW = 23;
	//用于统计功能表用户行为的统计。
	...
	public static final int USER_ACTION_TWENTY = 20;
	public static final int USER_ACTION_TWENTYONE = 21;
	public static final int USER_ACTION_TWENTYTWO = 22;
	//”GO桌面引导页“推荐GO精品主题
	...
	public static Map<String, ?> getUserActionData(String type) {
		PreferencesManager sp = new PreferencesManager(ApplicationProxy.getContext(), type,
				Context.MODE_PRIVATE);
		return sp.getAll();
	}
	/**
	...
	private static void countUserActionDataSync(final int actionID, final int action,
		...
		synchronized (sLocker) {
			...
			PreferencesManager sp = new PreferencesManager(ApplicationProxy.getContext(), actionType,
					Context.MODE_PRIVATE);
	...
	private static void countThemeTabDataSync(int tabId) {
		synchronized (sLocker) {
			...
			PreferencesManager sp = new PreferencesManager(ApplicationProxy.getContext(),
...
//		}
			...
			if (System.currentTimeMillis() - time < 30 * 60 * 1000L) {
				...
				if (type == AdvertConstants.ADVERT_STATISTICS_TYPE) {
					MsgMgrProxy.sendMessage(null, IDiyFrameIds.SCREEN_ADVERT_BUSINESS,
							IScreenAdvertMsgId.REQUEST_ADVERT_STAT_INSTALL, -1, appname);
				} else if (type == AdvertConstants.ADVERT_STATISTICS_GOWIDGET_TYPE) {
					// 如果是推荐widget，则调用widget接口
					MsgMgrProxy.sendMessage(null, IDiyFrameIds.SCREEN_ADVERT_BUSINESS,
							IScreenAdvertMsgId.REQUEST_ADVERT_STAT_INSTALL_WIDGET, -1, appname);
				}
	...
	public static void backUpLanguageSetting(Context context) {
		...
		String currentlanguage = sharePreferences.getString(IPreferencesIds.CURRENT_SELECTED_LANGUAGE,
				"");
		sharePreferences.putString(IPreferencesIds.CURRENT_SELECTED_LANGUAGE_BACKUP, currentlanguage);
	...
	public static void restoreLanguageSetting(Context context) {
		...
		String currentlanguageBack = sharePreferences.getString(
				IPreferencesIds.CURRENT_SELECTED_LANGUAGE_BACKUP, "");
		sharePreferences.putString(IPreferencesIds.CURRENT_SELECTED_LANGUAGE, currentlanguageBack);
		sharePreferences.commit();
	}
