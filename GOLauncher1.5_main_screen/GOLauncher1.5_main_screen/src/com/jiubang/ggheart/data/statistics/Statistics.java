...
public class Statistics {
	...
	private static String sUID = null; // 桌面UID
...
//	private static final String STATISTICS_GOLOCKER_USING_THEME_PKG_NAME = "usingThemePackageName"; // 保存网络请求成功使用时间所使用的KEY
	...
	//内置的Go开关widget包名
	public static final String GOSWITCH_WIDGET_PACKAGE_NAME = "com.gau.go.launcherex.gowidget.goswitch";
	
	//内置的天气widget包名
	public static final String WEATHER_WIDGET_PACKAGE_NAME = "com.gau.go.launcherex.gowidget.goweather";
...
//		}
	...
	private String getPublicStatisticsData() {
		...
		//goid
		statisticsDataStringBuffer.append(STATISTICS_DATA_SEPARATE_STRING).append(
				StatisticsManager.getGOID(mContext));
	...
	private String getUserActionStatisticsData() {
		...
		if (mContext != null) {
			...
			ShortCutSettingInfo shortCutSettingInfo = SettingProxy.getShortCutSettingInfo();
	...
	private String getUserDeskSettingStatData() {
		...
		if (mContext != null) {
			...
			ShortCutSettingInfo shortCutSetting = SettingProxy.getShortCutSettingInfo();
			ScreenSettingInfo screenSetting = SettingProxy.getScreenSettingInfo();
			DesktopSettingInfo desktopSetting = SettingProxy.getDesktopSettingInfo();
			ScreenStyleConfigInfo screenStyleInfo = ThemeManager.getInstance(mContext).getScreenStyleSettingInfo();
			GravitySettingInfo gravitySetting = SettingProxy.getGravitySettingInfo();
			EffectSettingInfo effectSetting = SettingProxy.getEffectSettingInfo();
			ThemeSettingInfo themeSetting = SettingProxy.getThemeSettingInfo();
											...
											case 5 :
												...
												if (tabHomeBg.equals(curTheme)) {
													data = "1";
														...
														|| tabHomeBg
																.equals(IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3)) {
													data = "2";
										...
										switch (j) {
											...
											case 7 :
												...
												if (info.equals(THEME_DEFAULT)
														|| info.equals(IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3_NEWER)
														|| info.equals(IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3)) {
													data = "1";
												}
												statisticsDataBuffer.append(headLevel3)
														.append(data)
									...
									for (int j = 1; j <= 3; j++) {
										...
										switch (j) {
											...
											case 3 :
												// 文字显示设置—字体
												data = "2";
												FontBean font = FontControler.getInstance(ApplicationProxy.getContext())
														.getUsedFontBean();
						...
						for (int o = 1; o <= 9; o++) {
							...
							switch (o) {
								case 1 :
									...
									if (type == OrientationTypes.VERTICAL) {
										data = "2";
									} else if (type == OrientationTypes.HORIZONTAL) {
										data = "3";
									} else if (type == OrientationTypes.IGNOREBORAD) {
										data = "4";
									}
...
//										}
	...
	private String getDeskEffectStatisticsData() {
		...
		if (mContext != null) {
			...
			// 统计6，屏幕切换类型
			EffectSettingInfo effectInfo = SettingProxy.getEffectSettingInfo();
...
//	}
	...
	private String getGestureActionStatisticsData() {
		...
		if (mContext != null) {
			...
			// HOME键
			int gestureId = 0;
			GestureSettingInfo gHomeInfo = SettingProxy.getGestureSettingInfo(GestureSettingInfo.GESTURE_HOME_ID);
			...
			// 上滑
			GestureSettingInfo gUpInfo = SettingProxy.getGestureSettingInfo(GestureSettingInfo.GESTURE_UP_ID);
			...
			statisticsDataBuffer.append(gestureId).append(STATISTICS_DATA_SEPARATE_STRING);
			// 下滑
			GestureSettingInfo gDownInfo = SettingProxy.getGestureSettingInfo(GestureSettingInfo.GESTURE_DOWN_ID);
	 ...
	 * @return
	 */
	private String getShortStatisticsData() {
		// TODO Auto-generated method stub
		ShortCutSettingInfo shortCutSettingInfo = SettingProxy.getShortCutSettingInfo();
		String statisticsData = null;
		if (shortCutSettingInfo != null) {
				...
				for (String key : keys) {
					...
					if (item != null && item.length > 1) {
						...
						if (clickCoutn > 0 && installCoutn == 0) {
							if (GoAppUtils.isAppExist(mContext, key)) {
								installCoutn++;
							}
						}
...
//		}
		...
		// 获取快捷条统计
		String shortStatisticsData = getShortStatisticsData();
	...
	private String getDisplaySettingStatisticsData() {
		...
		ScreenStyleConfigInfo screenStyleConfigInfo = ThemeManager.getInstance(mContext).getScreenStyleSettingInfo();
		...
		// 屏幕切换效果
		EffectSettingInfo effectSettingInfo = SettingProxy.getEffectSettingInfo();
			...
			if (allGoWidgetInfosMap != null && allGoWidgetInfosMap.size() > 0) {
				...
				for (String key : keySet) {
					...
					if (goWidgetProviderInfo != null) {
						// 获取包名
						packageName = goWidgetProviderInfo.getPkgName();
...
//		if (mContext != null) {
//			// GO锁屏主程序统计信息
//			if (GoAppUtils.isAppExist(mContext, new Intent(ICustomAction.ACTION_LOCKER))) {
//				StringBuffer statisticsDataBuffer = new StringBuffer();
//				// 如果锁屏主程序有安装
//				// 一级功能点ID和二级功能点ID
...
//		GameCenterStatisticsUtil.delRecords(mContext);
	...
	public static String language(Context context) {
		...
		try {
			ret = String.format("%s-%s", Locale.getDefault().getLanguage(), Locale.getDefault().getCountry());
		} catch (Throwable e) {
			//			 e.printStackTrace();
		}
	...
	public static String simlanguage(Context context) {
		...
		try {
			ret = String.format("%s-%s", Locale.getDefault().getLanguage(), getSimCountry(context));
		} catch (Throwable e) {
			//			 e.printStackTrace();
		}
	...
	public static boolean is200ChannelUid(Context context) {
		...
		return (uid != null && (uid.equals("200") || uid.equals("373") || uid.equals("218")
				|| ((uid.length() == 4) && uid.startsWith("9"))
				|| uid.equals("250") || uid.equals("299"))) ? true : false;
	}
...
//	private static String getLineSepSafe() {
//		String lineSep = null;
//		try {
...
//	}
	...
	private static void saveDeviceIdToSharedpreference(Context context, String deviceId) {
		...
		pref.putString(PrefConst.KEY_RANDOM_DEVICE_ID, deviceId);
		pref.commit();
	}
	/**
	 * 获取随机生成的IMEI的方法
	...
	private static String getDeviceIdFromSharedpreference(Context context) {
		PrivatePreference pref = PrivatePreference.getPreference(context);
		return pref.getString(PrefConst.KEY_RANDOM_DEVICE_ID, DEFAULT_RANDOM_DEVICE_ID);
	}
	/**
	 * 更新桌面使用时间的统计数据的方法
	 */
	...
	public static void setUserCover(Context context, String isNew) {
		try {
			PrivatePreference pref = PrivatePreference.getPreference(context);
			pref.putString(PrefConst.KEY_USER_IS_COVER, isNew);
			pref.commit();
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	public static String getUserCover(Context context) {
		...
		try {
			PrivatePreference pref = PrivatePreference.getPreference(context);
			isCover = pref.getString(PrefConst.KEY_USER_IS_COVER, "1");
		} catch (Exception e) {
			// TODO: handle exception
		}
		return isCover;
