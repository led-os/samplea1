		...
		try {
			...
			if (null != cursor && cursor.moveToFirst()) {
				...
				do {
					...
					// 假图标不进行统计
					if (PackageName.GO_STORE_PACKAGE_NAME.equals(tmpStr)
							|| PackageName.GO_THEME_PACKAGE_NAME.equals(tmpStr)
							|| PackageName.GO_WIDGET_PACKAGE_NAME.equals(tmpStr)) {
						continue;
					}
					// 内存中没有，则新增一个。
...
//					pkgName = info.activityInfo.packageName.toString();
	...
	private static void addSpecialApps(Context context, List<Data> datas) {
		//应用中心
		ComponentName com = new ComponentName(PackageName.RECOMMAND_CENTER_PACKAGE_NAME,
				ICustomAction.ACTION_FUNC_SHOW_RECOMMENDCENTER);
		addSpecialApp(context, datas, com);
		
		//游戏中心
		ComponentName gameComp = new ComponentName(PackageName.GAME_CENTER_PACKAGE_NAME,
	...
	public static long getApkSize(PackageManager pm, String packageName) {
