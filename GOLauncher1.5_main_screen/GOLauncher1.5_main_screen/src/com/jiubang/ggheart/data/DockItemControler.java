...
public class DockItemControler extends Controler implements ICleanable {
	...
	private void initData() {
		...
		dockAppItemInfo.mIconPackage = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
		...
		dockAppItemInfo.mTitle = ApplicationProxy.getContext().getString(R.string.customname_dial);
		...
		dockAppItemInfo.mIcon = AppDataEngine.getInstance(mContext).getSysBitmapDrawable();
		dockAppItemInfo.mTitle = ApplicationProxy.getContext().getString(R.string.customname_contacts);
		mDockAppItemInfos.add(dockAppItemInfo);
		...
		dockAppItemInfo.mIcon = AppDataEngine.getInstance(mContext).getSysBitmapDrawable();
		dockAppItemInfo.mTitle = ApplicationProxy.getContext()
		...
		dockAppItemInfo.mIcon = AppDataEngine.getInstance(mContext).getSysBitmapDrawable();
		dockAppItemInfo.mTitle = ApplicationProxy.getContext().getString(R.string.customname_sms);
		mDockAppItemInfos.add(dockAppItemInfo);
		...
		dockAppItemInfo.mIcon = AppDataEngine.getInstance(mContext).getSysBitmapDrawable();
		dockAppItemInfo.mTitle = ApplicationProxy.getContext().getString(R.string.customname_browser);
		mDockAppItemInfos.add(dockAppItemInfo);
	}
		...
		for (int i = 0; i < 5; i++) {
			...
			if (null != drawable) {
				if (drawable instanceof BitmapDrawable) {
					...
					bitmapDrawable = BitmapUtility.createBitmapDrawableFromDrawable(drawable, mContext);
				}
			}
	...
	public static DeskThemeBean.SystemDefualtItem getSystemDefualtItem(String themePkg, int index) {
		...
		if (index < DockUtil.ICON_COUNT_IN_A_ROW) {
			DockBean bean = DockChangeIconControler.getInstance(ApplicationProxy.getContext())
	...
	public AppItemInfo getDockDial() {
		return mDockAppItemInfos.get(0);
	}
	
	public AppItemInfo getDockSms() {
		return mDockAppItemInfos.get(3);
	}
