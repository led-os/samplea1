	...
	public void installUninstallListener(Context context) {
		...
		mFunAppReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				// 以下代码必须干掉
				/**
				 * 这里处理的情况是为了处理与桌面主题同样包名的GO桌面美化版的情况 Ad by shenjinbao
				 */
//				boolean isLauncherReplaceTheme = false;
				/*
				 * 这里需要分隔第三方应用的更新情况，因为主题的包名命名规则都是有规律的，
				 * 要么是ThemeManager.MAIN_THEME_PACKAGE这个开头
				 * ，要么是ThemeManager.OLD_THEME_PACKAGE
				 */
//				if (replacing
//						&& (packageName
//								.contains(ThemeManager.MAIN_THEME_PACKAGE) || packageName
//								.contains(ThemeManager.OLD_THEME_PACKAGE))) {
//
//					isLauncherReplaceTheme = true;
//					Intent intentTheme = new Intent(
//							ThemeManager.MAIN_THEME_PACKAGE);
...
//						if (themes.get(i).activityInfo.packageName
//								.equals(packageName)) {
...
//				}
				...
				if (!replacing && Intent.ACTION_PACKAGE_ADDED.equals(action)) {
...
//					}
					...
					installFlag = FLAG_CHANGE;
					// add by huyong 2013-01-30 for
					// 三星Note机测试（泽润）发现，安装桌面启动后，会收到ACTION_PACKAGE_CHANGED消息，在widget检查时，进而引起桌面重启，这里做过滤本桌面包处理
					...
					installFlag = FLAG_UPDATE;
//					// 覆盖安装，检查当前应用的是否是超级主题，若是，则重启桌面
//					ThemeManager themeManager = ThemeManager
//							.getInstance(mContext);
//					if (themeManager != null) {
//						themeManager.handleMaskThemeUpdated(packageName);
//					}
				}
				...
				Log.d(MONITORSVERLOG, "Monitor package Change flag = "
						+ installFlag);
		...
		mExternalAppsReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				if (action.equals(ICustomAction.ACTION_EXTERNAL_APPLICATIONS_AVAILABLE)) {
					// 通知
					broadCast(EXTERNAL_APP_AVAILABLE, 0, intent);
				} else if (action.equals(ICustomAction.ACTION_EXTERNAL_APPLICATIONS_UNAVAILABLE)) {
					// 通知
					broadCast(EXTERNAL_APP_UNAVAILABLE, 0, intent);
				}
			}
		};
		try {
	...
	private void checkDebugHelperInstalled(String pkgName) {
		if (PackageName.GO_DEBUG_HELPER_PKGNAME.equals(pkgName)
				&& AppUtils.getVersionCodeByPkgName(mContext, PackageName.GO_DEBUG_HELPER_PKGNAME) > 1) {
			//启动debug
