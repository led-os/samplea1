...
public class ShortCutInfo extends FeatureItemInfo {
	...
	public void onBCChange(int msgId, int param, Object ...object) {
		switch (msgId) {
			case AppItemInfo.INCONCHANGE : {
				// 更新图标缓存
				if (!mIsUserIcon && object != null && object[0] instanceof Drawable) {
					mIcon = (Drawable) object[0];
				}
				break;
			}
			...
			case AppItemInfo.TITLECHANGE : {
				// 更新标题缓存
				if (!mIsUserTitle && object != null && object[0] instanceof String) {
					mTitle = (String) object[0];
				}
				break;
			}
			...
			case AppItemInfo.UNREADCHANGE : {
				...
				mCounter = param; // 移动位置，解决快捷方式不能实时更新的问题
				break;
			}
	...
	public int getUnreadCount() {
		AppItemInfo appitem = getRelativeItemInfo();
		if (appitem != null) {
			return appitem.getUnreadCount();
		}
		return 0;
	}
	...
	public boolean equals(ItemInfo info) {
		...
		if (info instanceof ShortCutInfo) {
			retval = equals((ShortCutInfo) info);
}
		return retval;
	}
	...
	public boolean equals(ShortCutInfo info) {
		...
		if (info.mIntent != null) {
			if (info.mItemType == IItemType.ITEM_TYPE_APPLICATION
					&& this.mIntent.getComponent() != null) {
				// 通过componentName比较，不用转String
				retval = ConvertUtils.intentCompare(this.mIntent, info.mIntent);
			} else {
				// 如果componentName都为null,则通过转String比较
				retval = ConvertUtils.intentToStringCompare(this.mIntent, info.mIntent);
			}
		}
		return retval;
