	...
	public boolean prepareFeatureIcon() {
		if (ImagePreviewResultType.TYPE_IMAGE_FILE == mFeatureIconType) {
			try {
				...
				icon.setTargetDensity(ApplicationProxy.getContext().getResources().getDisplayMetrics());
				mFeatureIcon = icon;
			...
			try {
				...
				Bitmap bitmap = BitmapUtility.loadBitmap(ApplicationProxy.getContext(), uri, 1);
				if (null != bitmap) {
					int sz = 0;
					...
					BitmapDrawable icon = new BitmapDrawable(ApplicationProxy.getContext()
							.getResources(), bitmap);
					mFeatureIcon = icon;
				}
			} catch (OutOfMemoryError e) {
			...
			mFeatureIcon = ImageExplorer.getInstance(ApplicationProxy.getContext()).getDrawable(
