...
public class GoLockerThemeManager {
	...
	public Map<CharSequence, CharSequence> queryInstalledTheme() {
		...
		//		if (!GoAppUtils.isAppExist(mContext, LauncherEnv.GO_LOCK_PACKAGE_NAME)) {
		//			return themePackage;
		//		}
	...
	public boolean changeLockTheme(CharSequence newThemePkgName) {
		try {
			if (newThemePkgName != null) {
				Intent intent_GL = new Intent(
						ICustomAction.ACTION_SEND_TO_GOLOCK_FOR_GOLOCKER);
				...
				MsgMgrProxy.sendMessage(this, IDiyFrameIds.SCREEN,
						IScreenFrameMsgId.GO_LOCKER_PRECHANGE, -1, null, null);
				new Handler().postDelayed(new Runnable() {
					@Override
					public void run() {
						MsgMgrProxy.sendMessage(this,
								IDiyFrameIds.SCREEN,
								IScreenFrameMsgId.GO_LOCKER_CHANGED, -1, null,
								null);
					}
				}, 800);
			}
