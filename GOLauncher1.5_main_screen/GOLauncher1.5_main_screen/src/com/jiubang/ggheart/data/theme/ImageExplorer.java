...
public class ImageExplorer implements IExplorer {
	...
	private String mCurrentPackageName = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
	...
	public Drawable getDrawable(String drawableName) {
		String themePackageName = mCurrentPackageName;
	...
	public int getResourceId(String drawableName) {
		...
		int id = -1;
		String themePackageName = mCurrentPackageName;
		if (IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3.equals(themePackageName)) {
			themePackageName = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
		}
		...
		if (id == -1) {
			try {
				...
				if (IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3.equals(themePackageName) 
						|| AppUtils.isAppExist(mContext, themePackageName)) {
	...
	public int getResourceId(String drawableName, String packageName) {
		...
		if (IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3.equals(packageName)) {
			packageName = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
		}
		try {
			Resources themeResources = null;
			
			if (IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3.equals(packageName) 
					|| AppUtils.isAppExist(mContext, packageName)) {
				themeResources = mContext.getPackageManager().getResourcesForApplication(
						packageName);
			} else {
	...
	public Drawable getDrawable(String themePackage, String drawableName) {
		...
		try {
			Resources themeResources = null;
			if (IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3.equals(themePackage) 
					|| AppUtils.isAppExist(mContext, themePackage)) {
				themeResources = mContext.getPackageManager().getResourcesForApplication(
						themePackage);
			} else {
	...
	public Drawable getDrawableForDensity(Resources paramResources, int id) {
		...
		if (mCanDrawableForDensity) {
			try {
				...
				localObject = (Drawable) mLocalMethod.invoke(paramResources, Integer.valueOf(id),
						Integer.valueOf(240));
				return localObject;
		...
		return getDrawableForDensity2(paramResources, id);
	}

	public Drawable getDrawableForDensity2(Resources paramResources, int id) {
		Drawable drawable = paramResources.getDrawable(id);
		if (drawable instanceof NinePatchDrawable || drawable instanceof StateListDrawable) {
			...
			try {
				...
				localDisplayMetrics.density = DrawUtils.sDensity;
				...
				localOptions.inTargetDensity = (int) (160 * DrawUtils.sDensity);
