...
public class PaidThemeInfoGetter {
	//正式地址
	private static String sPaidThemeInfoUrl = LauncherEnv.Server.PAID_THEME_INFO_URL_PRO;
    //文件保存路径
    ...
    static {
		if (ServerUtils.isUseTestServer(LauncherEnv.Server.PAID_THEME_INFO_CONFIG_USE_TEST_SERVER)) {
			sPaidThemeInfoUrl = LauncherEnv.Server.PAID_THEME_INFO_URL_SIT;
		}
	}
	...
	public static void getPaidThemesInfo(final Context context) {
		new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
			public void run() {
				...
				final String url = sPaidThemeInfoUrl + uid + "&lang=" + lang;
				if (DEBUG) {
					Log.i(TAG, "url = " + url);
				}
				try {
