...
public class XmlParserFactory {
	...
	public static XmlPullParser createXmlParser(Context context, String fileName,
		...
		if (ctx != null && fileName != null) {
			try {
				...
				Log.i("XmlParserFactor", "NameNotFoundException for " + themePackage);
			}
	...
	public static InputStream createEncryptXmlInputStream(Context context, String packageName,
		...
		try {
			...
			if (inputStream == null) {
				Context ctx = AppUtils.getAppContext(context, packageName);
				int id = ctx.getResources().getIdentifier(xmlName, "raw", packageName);
				if (id != 0) {
					try {
