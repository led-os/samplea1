...
public class ThemeInfoBean extends ThemeBean implements Parcelable {
	...
	private boolean mHasWallpaper = false; //是否有壁纸（主要是用来判断atom主题是否有wallpaper.xml文件）

	private int mMinGOLauncherVersion = -1; // 最低桌面版本号
	private String mThemeType = null;
	private int mDrawModel = 0;
	...
	private String mGALink = null; // google分析链接
	
	private String mIconUrl; // 小图标
	/**
	 * 类描述:
	 * 功能详细描述:
	 * @date  [2012-9-28]
	...
	public ThemeInfoBean() {
		...
		// TODO:默认配置
		mThemeName = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
	}
    ...
    public void setmDrawModel(int mDrawModel) {
        this.mDrawModel = mDrawModel;
    }
	...
	public void writeToParcel(Parcel dest, int flags) {
		...
		dest.writeInt(mFeaturedId);
		dest.writeString(mThemeName);
		dest.writeString(mDownurl);
	}
	...
	public static final Parcelable.Creator<ThemeInfoBean> CREATOR = new Creator() {
		...
		public ThemeInfoBean createFromParcel(Parcel source) {
			...
			p.mFeaturedId = source.readInt();
			p.mThemeName = source.readString();
			p.mDownurl = source.readString();
			return p;
		}
	...
	public void setHasWallpaper(boolean mHasWallpaper) {
		this.mHasWallpaper = mHasWallpaper;
	}
