 ...
 *
 */
public class GoSettingDataModel extends DataModel implements ICleanable, Parcelable {
	...
	public GravitySettingInfo getGravitySettingInfo() {
		...
		if (null != cursor) {
			try {
				GravitySettingInfo info = new GravitySettingInfo(mContext);
				boolean bOk = info.parseFromCursor(cursor);
				if (bOk) {
					return info;
				}
	 ...
	 * @return
	 */
	public HashMap<Integer, String> getAppFuncSettings(String pkname) {
		HashMap<Integer, String> settingValues = new HashMap<Integer, String>();
		Cursor cursor = mDataProvider.getAppFuncSettingValues(pkname);
		if (cursor != null) {
			try {
				if (cursor.moveToFirst()) {
					int keyIndex = cursor.getColumnIndex(AppFuncSettingTable.SETTINGKEY);
					int valueIndex = cursor.getColumnIndex(AppFuncSettingTable.SETTINGVALUE);
...
//	}
	...
	public void addAppFuncSetting(final String packageName) {
		...
		if (packageName.compareTo(IGoLauncherClassName.DEFAULT_THEME_PACKAGE) == 0) {
		...
		try {
			...
			map.put(Setting.MENUAPPSTYLE, String.valueOf(AppSettingDefault.MENUAPPSTYLE));
			map.put(Setting.TURNSCREENDIRECTION, String.valueOf(AppSettingDefault.TURNSCREENDIRECTION));
			map.put(Setting.APPNAMEVISIABLE, String.valueOf(AppSettingDefault.APPNAMEVISIABLE));
			map.put(Setting.LINECOLUMNNUM, String.valueOf(AppSettingDefault.LINECOLUMNNUM));
			map.put(Setting.BACKGROUNDPICPATH, String.valueOf(AppSettingDefault.BACKGROUNDPICPATH));
			map.put(Setting.SORTTYPE, String.valueOf(AppSettingDefault.SORTTYPE));
			map.put(Setting.SHOWNEGLECTAPP, String.valueOf(AppSettingDefault.SHOWNEGLECTAPPS));
			map.put(Setting.INOUTEFFECT, String.valueOf(AppSettingDefault.INOUTEFFECT));
			map.put(Setting.ICONEFFECT, String.valueOf(AppSettingDefault.ICONEFFECT));
			map.put(Setting.SCROLL_LOOP, String.valueOf(AppSettingDefault.SCROLL_LOOP));
			map.put(Setting.BLUR_BACKGROUND, String.valueOf(AppSettingDefault.BLUR_BACKGROUND));
			map.put(Setting.SHOW_TAB_ROW, String.valueOf(AppSettingDefault.SHOW_TAB_ROW));
			map.put(Setting.VERTICAL_SCROLL_EFFECT, String.valueOf(AppSettingDefault.VERTICAL_SCROLL_EFFECT));
			map.put(Setting.TAB_HOME_BG, packageName);
			map.put(Setting.INDICATOR_STYLE, packageName);
			if (packageName.compareTo(IGoLauncherClassName.DEFAULT_THEME_PACKAGE) == 0) {
				map.put(Setting.BGVISIABLE, String.valueOf(AppSettingDefault.BACKGROUNDVISIABLE - 1));
			} else {
				map.put(Setting.BGVISIABLE, String.valueOf(AppSettingDefault.BACKGROUNDVISIABLE));
			}
			...
			mDataProvider.addFuncSettingList(packageName, keys, values);
...
//	}
	 ...
	 *            设置信息
	 */
	public boolean updateShortCutSettingNonIndepenceTheme(ShortCutSettingInfo info) {
		ContentValues values = new ContentValues();
	...
	public void insertShortCutSetting(ShortCutSettingInfo info) {
		ContentValues values = new ContentValues();
		info.contentValues(values);
		String themeName = ThemeManager.getInstance(ApplicationProxy.getContext()).getCurThemePackage();
		if (null != themeName) {
	...
	//安全锁设置
		public DeskLockSettingInfo getDeskLockSettingInfo() {
			final DataProvider dataProvider = mDataProvider;
			Cursor cursor = dataProvider.queryDeskLockSetting();
			if (null != cursor) {
				try {
					DeskLockSettingInfo info = new DeskLockSettingInfo();
					boolean bOk = info.parseFromCursor(cursor);
					if (bOk) {
						return info;
					}
		...
		public void updateDeskLockSettingInfo(DeskLockSettingInfo info) {
			final DataProvider dataProvider = mDataProvider;
			ContentValues values = new ContentValues();
			info.contentValues(values);
			dataProvider.updateDeskLockSetting(values);
			values = null;
		}

		public void insertDeskLockSettingInfo(DeskLockSettingInfo info) {
			final DataProvider dataProvider = mDataProvider;
			ContentValues values = new ContentValues();
			info.contentValues(values);
			dataProvider.insertDeskLockSetting(values);
			values = null;
		}
		
		public void cleanDeskLockSettingInfo() {
			final DataProvider dataProvider = mDataProvider;
			dataProvider.clearTable(DeskLockable.TABLENAME);
		}
