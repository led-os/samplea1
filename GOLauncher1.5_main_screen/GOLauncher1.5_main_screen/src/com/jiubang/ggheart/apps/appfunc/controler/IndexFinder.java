...
public class IndexFinder {
	...
	public static int findFirstIndex(Context context, final List<? extends FunItemInfo> infos,
			final boolean inApps, int compareType, FunItemInfo value, final String order) {
		int result = -1;
		...
		for (int i = 0; i < size; ++i) {
			...
			switch (compareType) {
				case COMPARE_WITH_STRING :
					CompareTitleMethod method = new CompareTitleMethod();
					comp = method.doCompare(value, funItemInfo);
					break;
				case COMPARE_WITH_FREQUENCE :
					int clickedCount = value.getClickedCount(context);
	...
	public static int findFirstIndex(final Context context,
		...
		for (int i = 0; i < size; ++i) {
			...
			switch (compareType) {
				case COMPARE_WITH_STRING :
					CompareTitleMethod method = new CompareTitleMethod();
					comp = method.doCompare(item, funItemInfo);
					break;
	 ...
	 * @return
	 */
	public static int findIndexInList(final List<? extends FunItemInfo> infos, final FunItemInfo item) {

		int countIndex = 0;
		for (FunItemInfo funItemInfo : infos) {
			if (item == funItemInfo) {
				return countIndex;
			}
			countIndex++;
