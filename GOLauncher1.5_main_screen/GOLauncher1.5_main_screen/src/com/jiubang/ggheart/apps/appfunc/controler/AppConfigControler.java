...
public class AppConfigControler extends Controler implements ICleanable {
	...
	private AppConfigControler(Context context) {
		...
		mAppDataEngine = AppDataEngine.getInstance(ApplicationProxy.getContext());
	...
	public List<AppItemInfo> getHideApps() {
		Iterator<Entry<ComponentName, AppConfigInfo>> iter = mHideHashMap.entrySet().iterator();
		List<AppItemInfo> list = new ArrayList<AppItemInfo>();
		while (iter.hasNext()) {
			Entry<ComponentName, AppConfigInfo> entry = iter.next();
			AppConfigInfo val = entry.getValue();
			AppItemInfo info = mAppDataEngine.getAppItem(val.getIntent());
			if (info != null) {
				list.add(info);
			} else {
				// 如果为空，应不应该删除？不然的话可能有垃圾存在
				try {
					mAppConfigDataModel.delHideAppItem(val.getIntent());
		...
		//sort the list by title asc by default
		CompareTitleMethod sort = new CompareTitleMethod();
		SortHelper.doSort(list, sort);
		return list;
	}
}
