...
public class ScrollerViewGroup extends ViewGroup implements ScreenScrollerListener {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (currentView instanceof GridViewContainer || currentView instanceof GridSortContainer) {
			//头部的广告位
			View adBanner = null;
			if (currentView instanceof GridViewContainer) {
				GridViewContainer container = (GridViewContainer)currentView;
				adBanner = container.getHeadView();
			}
			else if (currentView instanceof GridSortContainer) {
				GridSortContainer container = (GridSortContainer)currentView;
				adBanner = container.getHeadView();
			}
			if (null != adBanner && adBanner.getVisibility() == View.VISIBLE) {
				float x = ev.getX();
				float y = ev.getY();
				View panel = adBanner;
				RectF rectF = new RectF(panel.getLeft(),
	...
	public int getScreenCount() {
		int count = 0;
		if (mScreenScroller != null) {
			count = mScreenScroller.getScreenCount();
		}
		return count;
	}
