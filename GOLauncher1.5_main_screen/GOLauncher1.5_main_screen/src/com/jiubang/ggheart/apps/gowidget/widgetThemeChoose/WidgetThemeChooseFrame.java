...
//	public static final String WIDGET_THEME_PACKAGE = "com.gau.go.launcherex.theme.gowidget";
	...
	private static boolean sDeskAlertDlgCanShow = true;
	...
	public WidgetThemeChooseFrame(Activity activity, IFrameManager frameManager, int id) {
		...
		//处理3D插件界面显示不全
		mLayout.setPadding(0, StatusBarHandler.getStatusbarVisibleHeight(), 0, 0);
		if (DrawUtils.isNavBarAvailable()) {
			mLayout.setPadding(0, 0, 0, DrawUtils.getNavBarHeight());

			if (!StatusBarHandler.isHide()) {
				mLayout.setPadding(0, StatusBarHandler.getStatusbarHeight(), 0,
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
					case EVENT_THEME_PARSE_OK : {
						...
						if (VIEW_TYPE_MORESKIN == viewHolder.mViewType) {
							viewHolder.mDownloadTxt.setText(null);
							viewHolder.mDownloadInfo.setText(null);
							// ((FrameLayout)view).removeView(viewHolder.downloadTxt);
							// ((FrameLayout)view).removeView(viewHolder.downloadInfo);

							viewHolder.mImagePreview.setClickable(false);
							viewHolder.mImagePreview.setFocusable(false);
						...
						if (null != widgetInfo) {
							if (widgetInfo.title != null) {
								viewHolder.mName.setText(widgetInfo.title);
							} else {
							...
							if (widgetInfo.resouces != null && widgetInfo.resouceId != 0) {
								try {
									...
									viewHolder.mImagePreview.setImageDrawable(drawable);
	...
	public void onAdd() {
		...
		GuideControler.getInstance(mContext).hideCloudViewByType(GuideControler.CLOUD_TYPE_SCREEN);
	}
	...
	public void onRemove() {
		...
		removeAndLeave();
		GuideControler.getInstance(mContext)
				.reshowCloudViewByType(GuideControler.CLOUD_TYPE_SCREEN);
	}
	...
	public boolean handleMessage(Object who, int msgId, int param, Object ...obj) {
		...
		switch (msgId) {
			...
			case ICommonMsgId.BACK_TO_MAIN_SCREEN : {
				// 移除自己
				MsgMgrProxy.postMessage(this, IDiyFrameIds.SCHEDULE_FRAME,
						IFrameworkMsgId.REMOVE_FRAME, getMsgHandlerId(), null, null);
				break;
			}
			case IWidgetMsgId.WIDGETCHOOSE_SKIN : {
				if (obj != null && obj[0] != null && obj[0] instanceof GoWidgetBaseInfo) {
					// TODO 应用主题
					mWidgetInfo = (GoWidgetBaseInfo) obj[0];
					if (!mIsParseOnce) {
						parseData();
						mIsParseOnce = true;
					}
	...
	public void onClick(View v) {
		if (mAddToDesktop == v && mClickAction == ClickAction.ApplyWidgetTheme) {
			...
			MsgMgrProxy.sendMessage(this, IDiyFrameIds.SCREEN,
					IScreenFrameMsgId.APPLY_GO_WIDGET_THEME, gowidgetId, bundle, null);
			MsgMgrProxy.sendMessage(this, IDiyFrameIds.SCHEDULE_FRAME, IFrameworkMsgId.REMOVE_FRAME,
					getMsgHandlerId(), null, null);
		...
		if (mAddToDesktop == v || (mImgDownloadView != null && v.equals(mImgDownloadView))) {
			if (mClickAction == ClickAction.UpgradeWidget) {
				...
				if (widgetPackName != null) {
					String uriString = MarketConstant.APP_DETAIL + widgetPackName;
					gotoMarketForAPK(uriString);
				}
			} else if (mClickAction == ClickAction.DownloadTheme) {
				// 更多主题
				final String uriString = MarketConstant.BY_KEYWORD + KEYWORD;
				GoAppUtils.gotoBrowserIfFailtoMarket(mContext, uriString,
		...
		if (mContext != null) {
			...
			try {
				...
				if (GoAppUtils.isCnUser()) {
					...
					if (!GoAppUtils.gotoMarket(mContext, uriString)) {
						// 如果跳转市场不成功
						gotoFtp(path);
					}
					MsgMgrProxy.sendMessage(this, IDiyFrameIds.SCHEDULE_FRAME,
		...
		if (sDeskAlertDlgCanShow) {
					...
					new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							if (GoAppUtils.isMarketExist(mActivity)) {
								GoAppUtils.gotoMarket(mActivity, linkArray[0]);
							} else {
			...
			deskAlertDialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
				...
				public void onDismiss(DialogInterface dialog) {
					sDeskAlertDlgCanShow = true;
					deskAlertDialog.selfDestruct();
				}
			});

			sDeskAlertDlgCanShow = false;
	...
	private void addMoreThemeView() {
		...
		try {
			Drawable downloadMark = mChooseView.getResources()
					.getDrawable(R.drawable.download_mark);
		...
		holder.mViewType = VIEW_TYPE_MORESKIN;

		holder.mImagePreview = mImgDownloadView;
		holder.mName = texView;
		holder.mGridView = gridview;

		holder.mDownloadTxt = downLoad;
		holder.mDownloadInfo = downLoadInfo;
	...
	private void setWidgetItemInfo(String packageName, String styleString, String themeFileName) {
		...
		for (int i = 0; i <= size; i++) {
			...
			if (appPackageName.equals(PackageName.RECOMMAND_GOTASKMANAGER_PACKAGE)) {
			...
			if (inputStream == null
					&& packageName.equals(PackageName.RECOMMAND_GOTASKMANAGER_PACKAGE)) {
	...
	public void addView(int position) {
		...
		if (widgetInfo.resouces != null && widgetInfo.resouceId != 0) {
			try {
				viewHolder.mImagePreview.setImageDrawable(widgetInfo.resouces
						.getDrawable(widgetInfo.resouceId));
			} catch (OutOfMemoryError e) {
				e.printStackTrace();
			}
		...
		ImageAdapter adpter = new ImageAdapter(mContext);
		adpter.changeResouce(mRow, mCol);
		viewHolder.mGridView.setAdapter(adpter);
	...
	class RecycleView {
		...
		public View getView() {
			...
			holder.mViewType = VIEW_TYPE_NORMSKIN;

			holder.mImagePreview = (ImageView) viewFrame.findViewById(R.id.widgetsubviewimage);

			holder.mName = (DeskTextView) viewFrame.findViewById(R.id.widgetstyletitle);

			holder.mGridView = (GridView) viewFrame.findViewById(R.id.widgetgridview);
	...
	private static class ViewHolder {
		 ...
		 * 更多主题界面的下载文字
		 */
		DeskTextView mDownloadTxt;

		/*
		 * 更多主题界面的下载信息
		 */
		DeskTextView mDownloadInfo;
	}
