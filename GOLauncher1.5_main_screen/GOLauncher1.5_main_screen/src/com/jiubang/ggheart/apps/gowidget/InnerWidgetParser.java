...
public final class InnerWidgetParser {
	...
	private static InnerWidgetInfo getInnerWidgetInfo(TypedArray a, Context context) {
		...
		String pkgName = a.getString(R.styleable.InnerWidget_widgetPackage);
		
		InnerWidgetInfo info = new InnerWidgetInfo(pkgName);
		...
		if (labelId > 0) {
			...
			info.getProviderInfo().label = resources.getString(R.string.go_store);
		...
		info.getProviderInfo().icon = a.getResourceId(R.styleable.InnerWidget_icon, 0);
		Log.d("zgq", "info.getProviderInfo().icon= " + info.getProviderInfo().icon);
		...
		if (info.mBuildin == InnerWidgetInfo.BUILDIN_ALL) {
			...
			info.mSettingList = a.getResourceId(R.styleable.InnerWidget_settinglist, 0);
		}
		return info;
	}
	...
	public static ArrayList<WidgetParseInfo> getWidgetParseInfos(Context context,
		...
		if (info != null && info.mBuildin == InnerWidgetInfo.BUILDIN_ALL) {
			...
			if (resources != null) {
				...
				try {
					// 获取图片
					String[] extras = resources.getStringArray(info.mPreviewList);
					if (extras == null || extras.length == 0) {
						try {
							Resources tempResources = context.getPackageManager().getResourcesForApplication(info.mWidgetPkg);
							if (tempResources != null) {
								resources = tempResources;
								extras = resources.getStringArray(info.mPreviewList);
							}
						} catch (Exception e) {
							e.printStackTrace();
						}
					...
					for (String extra : extras) {
						...
						if (res == 0) {
							res = resources.getIdentifier(extra, "drawable", context.getPackageName());
						}
						if (res != 0) {
					...
					for (String titl : titles) {
						...
						if (res == 0) {
							res = resources.getIdentifier(titl, "string", context.getPackageName());
						}
						if (res != 0) {
					...
					for (int w : widthIds) {
						WidgetParseInfo item = parseInfos.get(count);
//						item.minWidth = w;
						item.minWidth = DrawUtils.px2dip(CellUtils.measureGoWidgetMinWidth(context, item.mCol));
						count++;
					}
					...
					for (int h : heightIds) {
						WidgetParseInfo item = parseInfos.get(count);
//						item.minHeight = h;
						item.minHeight = DrawUtils.px2dip(CellUtils.measureGoWidgetMinHeight(context, item.mRow));
						count++;
					}
