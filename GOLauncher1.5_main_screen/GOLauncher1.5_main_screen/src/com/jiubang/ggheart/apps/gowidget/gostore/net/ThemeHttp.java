...
public class ThemeHttp {
	...
	private static final String PVERSION = "32"; // 请求协议版本号
	...
	private static void compoundNameValuePairs(Context context, List<NameValuePair> nameValuePairs,
		...
		if (context != null) {
			...
			if (nameValuePairs != null
					...
					&& nameValuePairs.get(1).getValue()
							.equals(String.valueOf(OnlineThemeGetter.THEME_APPUID))) {
				Log.d("zyz", "themehttp---compoundNameValuePairs locker");
				vip = ThemePurchaseManager.getCustomerLevel(context,
						ThemeManageView.LOCKER_THEME_VIEW_ID);
			} else {
				Log.d("zyz", "themehttp---compoundNameValuePairs launcher");
				vip = ThemePurchaseManager.getCustomerLevel(context,
			...
			basicNameValuePair = new BasicNameValuePair("iscoupon", String.valueOf(1));
			nameValuePairs.add(basicNameValuePair);
			
			basicNameValuePair = new BasicNameValuePair("goid", StatisticsManager.getGOID(context));
			nameValuePairs.add(basicNameValuePair);
		}
	}
