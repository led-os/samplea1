...
//						downloadStatus, ItemDetailActivity.START_TYPE_APPMANAGEMENT, null);
				...
				AppManagementStatisticsUtil.getInstance().saveUpdataClick(getContext(),
						appBean.mPkgName, appBean.mAppId, 0, appBean.mICallBackUrl);
	...
	protected void handleRequestChange(int state, Object value) {
		switch (state) {
			...
			case AppsManageViewController.MSG_ID_EXCEPTION : {
				...
				MsgMgrProxy.sendMessage(this, IDiyFrameIds.APPS_MANAGEMENT_UPDATE_APP_FRAME,
						IAppManagerMsgId.APPS_MANAGEMENT_OPERATION_BUTTON, 0, null, null);
...
//				}
//				// 修改应用更新tab头上更新的数字
//				MsgMgrProxy.sendMessage(this, IDiyFrameIds.APPS_MANAGEMENT_MAIN_FRAME,
...
//		AppsManagementActivity.getApplicationManager().setUpdateAppsCount(size);
		...
		if (size > 0) {
			...
			MsgMgrProxy.sendMessage(this, IDiyFrameIds.APPS_MANAGEMENT_UPDATE_APP_FRAME,
					IAppManagerMsgId.APPS_MANAGEMENT_OPERATION_BUTTON, 1, null, null);
		} else {
			//没有更新的应用，显示无更新应用的提示
			if (mAppsManageViewAdapter != null) {
				mAppsManageViewAdapter.setDataSet(null);
	...
	// boolean isExist = GoAppUtils.isAppExist(getContext(), bean.mPkgName);
	// if (!isExist) {
	// // 如果不存在，则加入临时列表
	// tmpList.add(bean);
	// }
			...
			for (AppBean appBean : appBeans) {
				if (appBean.mAppId == id) {
					switch (downloadState) {
						case IAppManagerMsgId.APPS_MANAGEMENT_WAIT_FOR_DOWNLOAD :
							if (appBean.getStatus() == AppBean.STATUS_CANCELING) {
								applicationManager.cancelDownload(appBean);
							} else {
								appBean.setStatus(AppBean.STATUS_WAITING_DOWNLOAD);
							...
							break;
						case IAppManagerMsgId.APPS_MANAGEMENT_START_DOWNLOAD :
							if (appBean.getStatus() == AppBean.STATUS_CANCELING) {
								applicationManager.cancelDownload(appBean);
							} else {
								appBean.setStatus(AppBean.STATUS_DOWNLOADING);
							...
							break;
						case IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOADING :
						...
						case IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_COMPLETED :
							...
							// }
							break;
						case IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_CANCELED :
							appBean.setAlreadyDownloadSize(0);
							appBean.setAlreadyDownloadPercent(0);
							appBean.setStatus(AppBean.STATUS_NORMAL);
							break;
						case IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_FAILED :
							appBean.setStatus(AppBean.STATUS_DOWNLOAD_FAILED);
							appBean.setAlreadyDownloadSize(0);
							appBean.setAlreadyDownloadPercent(0);
							DeskToast.makeText(getContext(),
