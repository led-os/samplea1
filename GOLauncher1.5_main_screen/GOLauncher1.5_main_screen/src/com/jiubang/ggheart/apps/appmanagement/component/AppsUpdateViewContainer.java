...
public class AppsUpdateViewContainer extends LinearLayout
										...
										for (AppBean appBean : appBeans) {
											...
											AppManagementStatisticsUtil.getInstance()
													.saveUpdataClick(getContext(),
															appBean.mPkgName, appBean.mAppId, 1, appBean.mICallBackUrl);
										}
									}
								}
							}
			...
			case AppsManageViewController.ACTION_START_REQUEST : {
				...
				if (value != null && value instanceof AppsBean) {
					...
					MsgMgrProxy.sendMessage(this,
							...
							IAppManagerMsgId.APPS_MANAGEMENT_RECOMMENDED_APP, 0, null,
							appsBean.mListBeans);
	...
	private void setAppState(ArrayList<AppBean> appBeanList) {
		IDownloadService mDownloadController = DownloadControllerProxy.getInstance().getDownloadController();
		try {
	...
	public void setmControlInfo(byte appIconControl, byte gostoreControl, byte appFuncMenuControl) {
		...
		preferences.putInt(AppGameConstants.APPICON_SHOW_MESSSAGE, appIconControl); // 功能表图标右上角
		preferences.putInt(AppGameConstants.GOSTORE_SHOW_MESSAGE, gostoreControl); // GO
																			// Store功能表图标右上角
		preferences.putInt(AppGameConstants.APPFUNC_APPMENU_SHOW_MESSAGE, appFuncMenuControl); // 功能表Menu菜单
	...
	public boolean handleMessage(Object who, int msgId, int param, Object ...obj) {
		switch (msgId) {
			case IAppManagerMsgId.APPS_MANAGEMENT_OPERATION_BUTTON :
	...
	public void updateDownloadState(int downloadState, DownloadTask downloadTask) {
		...
		if (downloadState == IAppManagerMsgId.APPS_MANAGEMENT_WAIT_FOR_DOWNLOAD
				|| downloadState == IAppManagerMsgId.APPS_MANAGEMENT_START_DOWNLOAD
				|| downloadState == IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOADING) {
			// 点击全部更新按钮后
			if (mOperationType == OPERATION_TYPE_OPERATING_UPDATE) {
				boolean has = hasGettingReadyApps();
				if (!has) {
		...
		} else if (downloadState == IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_COMPLETED
				|| downloadState == IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_FAILED
				|| downloadState == IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_CANCELED) {
			// 点击取消全部更新按钮后
			if (mOperationType == OPERATION_TYPE_OPERATING_CANCEL
					|| mOperationType == OPERATION_TYPE_CANCEL_ALL) {
				boolean has = hasDownloadingAndWaitingApps();
