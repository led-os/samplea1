...
import com.go.util.sort.CompareSizeMethod;
...
public class MyAppsListAdapter extends BaseAdapter implements PinnedHeaderAdapter {
	...
	ArrayList<AppItemInfo> mAppItemInfos = null;
	...
	public MyAppsListAdapter(Context context, int state) {
		...
		initHandler();
		mAppItemInfos = AppDataEngine.getInstance(ApplicationProxy.getContext()).getAllAppItemInfos();
		// loadListData();
	}
	...
	private synchronized void getListData() {
		...
		if (!internalAppsData.isEmpty()) {
			SortHelper.doSort(internalAppsData, method);
			internalAppSize = internalAppsData.size();
		}

		if (!externalAppsData.isEmpty()) {
			SortHelper.doSort(externalAppsData, method);
	...
	private void sortItem(ArrayList<AppInfo> allAppsDataList, int type) {
		...
		switch (type) {
			case 1 :
				method = new CompareTimeMethod(mContext);
			...
			case 2 :
				...
				break;
			case 3 :
				method = new CompareSizeMethod();
				method.setOrder(CompareMethod.DESC);
				break;
			default :
				break;
		}
		SortHelper.doSort(allAppsDataList, method);
	}
	...
	private BitmapDrawable getDrawableFromAppItemInfo(String pkgName) {
		if (pkgName == null || mAppItemInfos == null || mAppItemInfos.size() == 0) {
			return null;
		}
		for (AppItemInfo appItemInfo : mAppItemInfos) {
