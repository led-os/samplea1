...
//	public static final String ACTION_APP_MANAGEMENT_CN = "com.gau.go.launcherex.appmanagement.cn";
	 ...
	 * @param statEntrance
	 */
	public void show(int entranceId, int showViewId, int statEntrance) {
		if (mContext != null) {
			mShowWarningDialog = true;
			Intent intent = new Intent(ICustomAction.ACTION_APP_MANAGEMENT_CN);
			intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
	...
	public void getAllInstalledApp(ArrayList<AppInfo> internalApps, ArrayList<AppInfo> externalApps) {
		try {
			...
			if (appCount > 0) {
				...
				// AppDataEngine appDataEngine = AppDataEngine.getInstance(ApplicationProxy.getContext())();
	...
	// // }
	// }
	// // ApplicationProxy.getApplication().setHasInstalledTaskmanager(
...
//		mContext.startActivity(intent);
	...
	public String startDownload(AppBean appBean) {
		...
		String googleMarketurl = MarketConstant.APP_DETAIL + appBean.mPkgName;
	...
	public boolean handleMessage(Object who, int msgId, int param, Object ...obj) {
		...
		switch (msgId) {
			...
			case IAppDrawerMsgId.EVENT_APPS_LIST_UPDATE :
				...
				AppsBean appsBean = (AppsBean) obj[0];
				mUpdateAppsCount = appsBean.mListBeans.size();
				break;
			case IAppManagerMsgId.EVENT_APPS_LIST_UPDATE_NOTIFICATION :
				appsBean = (AppsBean) obj[0];
