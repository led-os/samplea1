...
public class MyAppsView extends PinnedHeaderListView {
	...
	// 界面调用类型
	public static final int VIEW_TYPE_APPS = 0; // 我的应用
	public static final int VIEW_TYPE_APPS_UNINSTALL = 1; // 批量卸载
	...
	public void initView() {
		...
		setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				// if
				// (!ApplicationProxy.getApplication().isHasInstalledTaskmanager())
				// {
				// if (!ApplicationProxy.getApplication().isHasShowMessage()) {
				// // 弹dialog来提示
				// onCreateDialog(view.getContext(), position);
				// ApplicationProxy.getApplication().setHasShowMessage(true);
				// return;
				// }
				// }
				// 如果是应用管理
