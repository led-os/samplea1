...
//	public static final String ACTION_APP_DOWNLOAD = "appsmanagement_download_info";
	...
	public void onStart(DownloadTask task) throws RemoteException {
		...
		//		MsgMgrProxy.sendHandler(this, IDiyFrameIds.APPS_MANAGEMENT_UPDATE_APP_FRAME,
		//				IDiyMsgIds.APPS_MANAGEMENT_START_DOWNLOAD, taskId, null, null);
		//		MsgMgrProxy.sendHandler(this, IDiyFrameIds.APPS_MANAGEMENT_RECOMMENDED_APP_FRAME,
		//				IDiyMsgIds.APPS_MANAGEMENT_START_DOWNLOAD, taskId, null, null);

		sendBroadcastingToAppCenter(IAppManagerMsgId.APPS_MANAGEMENT_START_DOWNLOAD, task);
	...
	public void onWait(DownloadTask task) throws RemoteException {
		...
		sendBroadcastingToAppCenter(IAppManagerMsgId.APPS_MANAGEMENT_WAIT_FOR_DOWNLOAD, task);
	}
	...
	public void onUpdate(DownloadTask task) throws RemoteException {
		...
		if (task != null) {
			...
			if (currentTime - mPrevRefreshTime > REFRESH_INTERVAL) {
				...
				sendBroadcastingToAppCenter(IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOADING, task);
	...
	public void onComplete(DownloadTask task) throws RemoteException {
		...
		if (task != null) {
			...
			sendBroadcastingToAppCenter(IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_COMPLETED, task);
			//			int taskId = Long.valueOf(task.getId()).intValue();
...
//					intent.setAction(android.content.Intent.ACTION_VIEW);
		...
		if (mContext != null && task != null) {
			...
			if (mRestarCount <= 5) {
				if (Machine.isNetworkOK(mContext)) {
					IDownloadService downloadController = DownloadControllerProxy.getInstance().getDownloadController();
	...
	public void onCancel(DownloadTask task) throws RemoteException {
		...
		if (task != null) {
			...
			sendBroadcastingToAppCenter(IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_CANCELED, task);
	...
	private void handleDownloadFail(DownloadTask downloadTask) {
		...
		sendBroadcastingToAppCenter(IAppManagerMsgId.APPS_MANAGEMENT_DOWNLOAD_FAILED, downloadTask);
