...
public class DeskSettingVisualIconTabView extends DeskSettingVisualAbsTabView {
	...
	protected void findView() {
		...
		mIconSize.setSummaryText(" ");
		mIconSize.setOnValueChangeListener(this);
		mIconSize.setOnListClickListener(this);
		mOriginalSize = IconUtilities.getIconSize(getContext());
		updateIconsPosition(GoLauncherLogicProxy.getIconSizeStyle(), mOriginalSize);
	...
	private void initIconShowFrame() {
		...
		if (DrawUtils.sWidthPixels - DrawUtils.dip2px(160) > lp.width) {
			lp.width = DrawUtils.sWidthPixels - DrawUtils.dip2px(160);
		}
		lp.height = mIconSizeMax;
		lp.topMargin = DrawUtils.dip2px(20);
		...
		lp = (RelativeLayout.LayoutParams) mMaxIcon.getLayoutParams();
		mMaxIcon.getLayoutParams();
		lp.topMargin = DrawUtils.dip2px(16);
		

		// 5:大图标标记text
		lp = (RelativeLayout.LayoutParams) mLargeIcon.getLayoutParams();
		lp.topMargin = mIconSizeMax - mLargeIconSize + DrawUtils.dip2px(13);
		mLargeIcon.setText(mLargeIcon.getText() + "(" + mLargeIconSize + ")");

		// 6:小图标标记text
		lp = (RelativeLayout.LayoutParams) mSmallIcon.getLayoutParams();
		lp.topMargin = mIconSizeMax - mSmallIconSize + DrawUtils.dip2px(13);
	...
	private void updateIconsPosition(int type, int screenIconSize) {
		...
		RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) mShowIconsFrame
				.getLayoutParams();
		if (DrawUtils.sWidthPixels - DrawUtils.dip2px(160) > frameWidth) {
			frameWidth = DrawUtils.sWidthPixels - DrawUtils.dip2px(160);
		}
		lp.width = frameWidth;
...
//			lp.bottomMargin = DrawUtils.dip2px(5);
...
//			lp.bottomMargin = DrawUtils.dip2px(5);
	...
	@Override
	public void changeOrientation(Configuration newConfig) {
		...
		dismissColorPicker();
		updateIconsPosition(GoLauncherLogicProxy.getIconSizeStyle(), mDeskIcon.getWidth());
	}
	...
	private void loadDeskIcon(String themePkg) {		
		...
		itemIntent.setComponent(new ComponentName("com.gau.go.launcherex", IGoLauncherClassName.GOLAUNCHER_ACTIVITY));
		
		AppDataThemeBean bean = AppDataEngine.getInstance(getContext()).createAppDataThemeBean(themePkg);
		AppDataFilter mAppDataFilter = new AppDataFilter(getContext(), bean);
		...
		try {
			...
			drawable = getContext().getPackageManager().getActivityIcon(itemIntent);

			if (drawable instanceof BitmapDrawable) {
				bitmapDrawable = (BitmapDrawable) drawable;
			} else {
				bitmapDrawable = BitmapUtility.createBitmapDrawableFromDrawable(drawable, getContext());
			}

			synchronized (mAppDataFilter) {
				cover = mAppDataFilter.getIconupon(true);
				mask = mAppDataFilter.getIconmask(true);
				final BitmapDrawable iconBase = mAppDataFilter.getIconback(true);
				if (iconBase != null) {
			...
			// 缩放图片
			if (drawable != null && bitmapDrawable.getBitmap() != null) {
				if (base != null || cover != null) {
					float scale = mAppDataFilter.getIconScaleFactor();
					drawable = IconUtilities.composeAppIconDrawable(base, cover, bitmapDrawable, mask , scale);
				} else {
					// drawable = new
					// BitmapDrawable(IconUtilities.createBitmapThumbnail(drawable.getBitmap(),
					// mContext));
					/*
					 * 默认主题没有底座，直接对取得的图标缩放成标准大小。但是如果图标已经是标准大小，
					 * IconUtilities.createBitmapThumbnail方法会直接返回原图。直接使用这些图标来绘图，
					 * 可能会受到系统共享保护之类的机制而导致绘制时间变长？例如功能表里面使用球和圆柱特效，
					 * 在某些屏之间切换会很慢，但是换其他特效又不会，确实很奇怪。 不管怎么样，这里保证了拷贝后，不会慢了。
					 */
					Bitmap bitmap = IconUtilities.createBitmapThumbnail(bitmapDrawable.getBitmap(), getContext());
					if (bitmap != null && bitmap == bitmapDrawable.getBitmap()) {
		...
		drawable = drawable != null ? drawable : getResources().getDrawable(R.drawable.golauncherex_4_def3);
		mDeskIcon.setImageDrawable(drawable);
	}
...
//		mDockIcon.setImageDrawable(drawable);
		...
		Drawable drawable = null;
		if (themePkg.equals(DockUtil.DOCK_DEFAULT_STYLE_STRING)) {
			drawable = getResources().getDrawable(R.drawable.shortcut_0_0_phone);
			mDockIcon.setImageDrawable(drawable);
			return;
		}
		...
		if (engine != null) {
			...
			if (bean != null) {
				...
				ImageExplorer explorer = ImageExplorer.getInstance(getContext());
		...
		drawable = drawable != null ? drawable : getResources().getDrawable(R.drawable.phone_4_def3);
		mDockIcon.setImageDrawable(drawable);
	}
	...
	private void loadFolderIcon(String themePkg) {
		Drawable drawable = null;
		if (themePkg.equals(PackageName.PACKAGE_NAME)) {
	...
	public void save() {
		if (null != mScreenStyleInfo && null != AppCore.getInstance()) {
			...
			if (null == style
				...
				MsgMgrProxy.sendBroadcastHandler(this,
						IAppCoreMsgId.REFRESH_SCREENICON_THEME, -1, null, null);
			}
			...
			if (null == style
				...
				mScreenStyleInfo.setFolderStyle(mFolderStyleIcon
						.getDeskSettingInfo().getSingleInfo().getSelectValue());
				MsgMgrProxy.sendBroadcastHandler(this,
						IAppCoreMsgId.REFRESH_FOLDER_THEME, -1, null, null);
			}
			...
			if (null == style
				...
				mScreenStyleInfo.setGGmenuStyle(mMenuStyleIcon
						.getDeskSettingInfo().getSingleInfo().getSelectValue());
				MsgMgrProxy.sendBroadcastHandler(this,
						IAppCoreMsgId.REFRESH_GGMENU_THEME, -1, null, null);
			}
		}
		...
		//
		// if (bChanged) {
		// SettingProxy.updateDesktopSettingInfo(
		// mDesktopInfo);
		// }
		// }
...
//						false);
		...
		if (mDesktopInfo != null
			...
			mDesktopInfo.mShowIconBase = mIconBase.getIsCheck();
			SettingProxy.updateDesktopSettingInfo2(
					mDesktopInfo, false);
			MsgMgrProxy.sendBroadcastHandler(this,
					IAppCoreMsgId.EVENT_SHOW_OR_HIDE_ICON_BASE,
	...
	private void initDockStyleRefEntry() {
		// 风格安装包扫描
		ArrayList<StyleBaseInfo> styleInfos = DockStyleIconManager.getInstance(ApplicationProxy.getContext()).getAllStyleBaseInfos();
	...
	private void initFolderStyleRefEntry() {
		...
		arrValues.add(IGoLauncherClassName.DEFAULT_THEME_PACKAGE);
	...
	private Comparator<ThemeInfoBean> mComparator = new Comparator<ThemeInfoBean>() {
		...
		public int compare(ThemeInfoBean object1, ThemeInfoBean object2) {
			...
			str1 = SortUtils.changeChineseToSpell(getContext(), str1, R.raw.unicode2pinyin);
			str2 = SortUtils.changeChineseToSpell(getContext(), str2, R.raw.unicode2pinyin);
	...
	private void initGGmenuStyleList() {
		...
		String curThemePkg = ThemeManager.getInstance(getContext())
				.getCurThemePackage();
		ArrayList<ThemeInfoBean> themeInfoBeans = ThemeManager.getInstance(ApplicationProxy.getContext()).getAllThemeInfosWithoutDefaultTheme();
		...
		entries[0] = getResources().getString(R.string.defaultstyle);
		values[0] = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
	...
	private void checkIconSizeChange() {
		...
		if (mFlag) {
			if (mDesktopInfo.getIconSizeStyle() != index || (index == DIY_ICON_SIZE)) {
				if (Machine.isTablet(ApplicationProxy.getContext()) && index == LARGE_ICON_SIZE) {
				...
				SettingProxy.updateDesktopSettingInfo2(mDesktopInfo, false);
				MsgMgrProxy.sendMessage(this, IDiyFrameIds.SCHEDULE_FRAME,
						ICommonMsgId.ACTIVITY_NEED_RESTART, 1, null, null);
			} else {
				MsgMgrProxy.sendMessage(this, IDiyFrameIds.SCHEDULE_FRAME,
						ICommonMsgId.ACTIVITY_NEED_RESTART, 0, null, null);
			}
		} else {
