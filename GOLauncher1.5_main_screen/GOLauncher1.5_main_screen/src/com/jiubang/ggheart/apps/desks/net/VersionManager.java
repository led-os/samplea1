...
import com.jiubang.ggheart.data.statistics.AdmobSwitchUtil;
...
public class VersionManager {
	...
	// //自动版本检测URL;
	private static String sAutoVersionCheckUrl = LauncherEnv.Server.AUTO_VERSION_CHECK_URL_PRO;
	...
	static {
		if (ServerUtils.isUseTestServer(LauncherEnv.Server.AUTO_VERSION_CHECK_CONFIG_USE_TEST_SERVER)) {
			sAutoVersionCheckUrl = LauncherEnv.Server.AUTO_VERSION_CHECK_URL_SIT;
		}
	}
	...
	private void initVersionCheckUnionProtocol() {
		mUnionProtocol = new UnionProtocol(ApplicationProxy.getApplication());
	}
	...
	public void autoCheckVersion() {
		if (mContext != null) {
			// 进行版本检查和上传统计数据
			versionCheckAndPostStatisticData(mContext, sAutoVersionCheckUrl);
...
//			UnionProtocol unionProtocol = new UnionProtocol(GoLauncher.getContext());
		...
		// 统计上传结束,进行广告开关状态更新
		AdmobSwitchUtil.updataAdmobSwitch(context);
	}
	...
	private class UnionNetCallBack extends UnionProtocol.ConnectListener {
		...
		public void onFinish(THttpRequest request, IResponse response) {
			...
			// 清除安装回调数据
			InstallCallbackManager.cleanSharePreferencesData();
	...
	public void saveCurTime() {
		// 保存当前时间值
		final ThemeSettingInfo info = SettingProxy.getThemeSettingInfo();
