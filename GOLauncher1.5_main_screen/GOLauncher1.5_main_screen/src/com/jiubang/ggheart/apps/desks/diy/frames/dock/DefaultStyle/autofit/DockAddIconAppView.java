...
public class DockAddIconAppView extends LinearLayout
	...
	public DockAddIconAppView(Context context, int type) {
		...
		initHandler();
		initData();
		mContentViewHeight = mContentLayout.getLayoutParams().height;
	}
	...
	public void initLayoutHeight() {
		...
		if (mContentLayout != null) {
			LauncherDialogUtils.setDialogWidth(mContentLayout, getContext());
			int limit = 0;
			int hLimit = 0;
			DisplayMetrics metrics = getContext().getResources().getDisplayMetrics();
			limit = (int) (metrics.heightPixels * 0.1f);
			hLimit = (int) (metrics.heightPixels * 0.75f);
			LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) mContentLayout
					.getLayoutParams();
...
//				LauncherDialogUtils.setDialogWidth(mContentLayout, mContext);
//			} else {
...
//			}
	...
	private void layoutData() {
		MutilCheckGridView gridView = mMultiCheckViewGroup.getGridView();
		if (DrawUtils.sDensity < 1.0 && gridView != null) {
			int count = 3;
			...
			gridView.setRowCount(count);
			gridView.setCellHeight(mMultiCheckViewGroup.getHeight() / count);
		}
	...
	public void initAppData() {
		...
		if (list2.size() > 0) {
			...
			SortHelper.doSort(list2, method);
...
//					shortCutInfo.mIcon = info.mIcon;
	...
	private void initGoShortCutData() {
		...
		for (int i = 0; i < size; i++) {
			...
			shortCutInfo.mIcon = IconUtilities.createIconThumbnail(getIcons(drawableIds[i]), mContext);
			shortCutInfo.mFeatureIconType = ImagePreviewResultType.TYPE_DEFAULT;
	...
	public void initDockDefaultIconData() {
		...
		for (int i = 0; i < name.length; i++) {
			...
			shortCutInfo.mFeatureIconPackage = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
			mAllList.add(shortCutInfo);
		}
	}
	...
	private Drawable[] getDockDefaultIcon() {
		...
		String pkg = SettingProxy.getShortCutSettingInfo().mStyle;
...
//		initLayout();
		...
		if (mInitOrientation == orientation) {
			mContentLayout.getLayoutParams().height = mContentViewHeight;
		} else {
			mContentLayout.getLayoutParams().height = getResources()
					.getDimensionPixelSize(
							R.dimen.dock_add_view_view_height);
		}
		initLayoutHeight();
		showProgressDialog();
		if (mHandler != null) {
