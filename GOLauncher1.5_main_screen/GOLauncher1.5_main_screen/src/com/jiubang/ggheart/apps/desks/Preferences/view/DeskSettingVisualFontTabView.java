...
public class DeskSettingVisualFontTabView extends DeskSettingVisualAbsTabView
	...
	@Override
	public boolean onValueChange(DeskSettingItemBaseView baseView, Object newValue) {
		if (mAppNameSize == baseView) {
			if (newValue instanceof String) {
				DeskSettingConstants.updateSingleChoiceListView(mAppNameSize, (String) newValue);
			} else if (newValue instanceof String[]) {
				String select = mAppNameSize.getDeskSettingInfo().getSingleInfo().getEntryValues()[1]
						.toString();
				DeskSettingConstants.updateSingleChoiceListView(mAppNameSize, select);
		...
		if (mFont == baseView) {
			if (newValue instanceof String) {
				DeskSettingConstants.updateSingleChoiceListView(mFont, (String) newValue);
				int selectInt = Integer.parseInt((String) newValue);
//				save();
				FontControler.getInstance(ApplicationProxy.getContext()).updateUsedFontBean(mFontBeans.get(selectInt));
	...
	private void loadFont() {
		FontControler control = FontControler.getInstance(ApplicationProxy.getContext());
		if (null != control) {
	...
	public void onFontScanChanged(ArrayList<FontBean> beans) {
		...
		// 写入数据库
		FontControler.getInstance(ApplicationProxy.getContext()).updateFontBeans(beans);
	...
	public void changeOrientation(Configuration newConfig) {
		...
		if (mFont != null) {
			mFont.changeOrientation(newConfig);
		}
	}
	...
	public boolean onPreValueChange(DeskSettingItemBaseView baseView,
			Object value) {
		if (baseView == mFont) {
			final DeskSettingItemListView view = (DeskSettingItemListView) baseView;
			final DeskSettingFontSingleInfo deskSettingFontSingleInfo = (DeskSettingFontSingleInfo) mFont
					.getDeskSettingInfo().getSingleInfo();
			final String current = String.valueOf(value);
			if (!"0".equals(current)) {
				...
				mScanFontConfirmDialog.setPositiveButton(
						...
						new View.OnClickListener() {
							...
							@Override
							public void onClick(View v) {
								deskSettingFontSingleInfo.setSelectValue(current);
								view.onDialogSelectValue(current);
							}
						});
				return true;
			}
