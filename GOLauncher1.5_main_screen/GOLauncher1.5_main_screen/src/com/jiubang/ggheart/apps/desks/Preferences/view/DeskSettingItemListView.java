...
public class DeskSettingItemListView extends DeskSettingItemBaseView
	...
	public DeskSettingItemListView(Context context, AttributeSet attrs) {
		...
		//分类
		else if (type == DialogTypeId.TYPE_DESK_SETTING_CLASSIFY_CHOICE) {
			DeskSettingClassifyInfo calssifyInfo = new DeskSettingClassifyInfo();
			calssifyInfo.setTitle(dialogTitle);
			mDeskSettingInfo.setClassifyInfo(calssifyInfo);
		}
		...
		if (mDeskSettingInfo != null) {
			switch (mDeskSettingInfo.getType()) {
				...
				case DialogTypeId.TYPE_DESK_SETTING_MULTICHOICE :
					...
					break;
				case DialogTypeId.TYPE_DESK_SETTING_CLASSIFY_CHOICE :
					DeskSettingClassifyInfo classifyInfo = mDeskSettingInfo.getClassifyInfo();
					if (classifyInfo != null) {
						super.setSummaryText(classifyInfo.getCurSingleChooseValueText());
					}
					break;
				default :
					break;
		...
		if (mDeskSettingInfo != null) {
			switch (mDeskSettingInfo.getType()) {
				...
				case DialogTypeId.TYPE_DESK_SETTING_CLASSIFY_CHOICE :
					DeskSettingClassifyInfo classifyInfo = mDeskSettingInfo.getClassifyInfo();
					if (classifyInfo != null) {
						selectValue = classifyInfo.getCurSingleChooseValue();
					}
					break;
				default :
					break;
	...
	/**
	 * <br>功能简述:设置二级菜单为多选(传入已经处理过的二级数组)
	 * <br>功能详细描述:
	 * <br>注意:需要设置父类引用
	 * @param entrisId 显示类容列表ID
	 * @param entryValuesId 显示内容值列表ID
	 * @param contentIndex 列表内容项的索引
	 * @param selectValue 勾选的值
	 * @param customPosition 自定义菜单在哪个位置（相对于一级菜单的数组）
	 */
	public void setSecondInfoMulti(CharSequence[] entries, CharSequence[] entryValues,
			int[] imageId, int[] selectValue, int customPosition, String title) {
		DeskSettingInfo customDeskSettingInfo = new DeskSettingInfo();
		customDeskSettingInfo.setType(DialogTypeId.TYPE_DESK_SETTING_MULTICHOICE); //设置多选类型
		String[] selectValues = DeskSettingConstants.intArray2StringArray(selectValue); //已勾选的值
		DeskSettingMultiInfo multiInfo = new DeskSettingMultiInfo(); //创建多选对象
		...
		multiInfo.setTitle(title); //设置多选标题
		multiInfo.setSelectValues(selectValues);

		if (imageId != null) {
			multiInfo.setImageId(imageId);
			multiInfo.setIsAddImageBg(true); //合成添加背景图片
		}
		customDeskSettingInfo.setMultiInInfo(multiInfo);
		customDeskSettingInfo.setParentInfo(mDeskSettingInfo); //设置子类的父类为当前的mDeskSettingInfo
	...
	public DeskSettingBaseDialog getDialog() {
		return mDialog;
	}
	 ...
	 * 设置分类菜单每个分类信息
	 */
	public void setClassifyDialogInfo(String title, Drawable gridItemIconBg,
			DeskSettingBaseInfo[] baseInfos, String selectValue,
			CharSequence multiValue, CharSequence multiValueText,
			int[] multiSelectValues, CharSequence[] multipleUnchooseValues) {
		DeskSettingClassifyInfo classifyInfo = mDeskSettingInfo
				.getClassifyInfo();
		if (classifyInfo == null) {
			classifyInfo = new DeskSettingClassifyInfo();
			mDeskSettingInfo.setClassifyInfo(classifyInfo);
		}
		classifyInfo.setCurSingleChooseValue(selectValue);
		if (selectValue != multiValue) {
			classifyInfo.setCurMultipleChooseValue(null);
		} else {
			classifyInfo.setCurMultipleChooseValue(ArrayUtils
					.intArrayToCharSequenceArray(multiSelectValues));
		}
		classifyInfo.setMultipleValue(multiValue);
		classifyInfo.setMultipleValueText(multiValueText);
		if (title != null && !title.equals("")) {
			classifyInfo.setTitle(title);
		}
		if (gridItemIconBg != null) {
			classifyInfo.setGridItemIconBg(gridItemIconBg);
		}
		if (baseInfos != null && baseInfos.length > 0) {
			for (DeskSettingBaseInfo info : baseInfos) {
				classifyInfo.addListItemInfo(info);
			}
		}
		if (multipleUnchooseValues != null) {
			classifyInfo.setMultipleUnchooseValues(multipleUnchooseValues);
		}
	}
	 ...
	 * @return
	 */
	public DeskSettingBaseInfo generateClassifyListItemInfo(
			CharSequence[] names, CharSequence[] values, int[] drawableIds,
			String title) {
		DeskSettingBaseInfo info = new DeskSettingBaseInfo();
		info.setEntries(names);
		info.setEntryValues(values);
		info.setImageId(drawableIds);
		info.setTitle(title);
		return info;
	}

	@Override
	public boolean onPreDialogSelectValue(Object value) {
		if (mOnValueChangeListener != null) {
			return mOnValueChangeListener.onPreValueChange(this, value);
		}
		return false;
	}
}
