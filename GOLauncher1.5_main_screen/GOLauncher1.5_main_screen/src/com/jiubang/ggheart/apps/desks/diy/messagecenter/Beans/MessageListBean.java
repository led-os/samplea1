...
public class MessageListBean {
	...
	public static final String TAG_QUIXEY = "quixey"; // 搜索界面是否开启使用Quixey的提示

	/**
	 * 
	 * 类描述: MessageHeadBean类 功能详细描述: 存放头消息的一些参数内容
	 * 
	 * @date [2012-9-28]
	 */
	public static class MessageHeadBean extends MessageBaseBean {
		...
		public boolean mIsDeleted = false; // 已读消息是否删除掉
		public int mViewType;
		public MessageDownLoadObserver mObserver;
		public Bitmap mBitmap; // 主题推送通知栏的Icon
		public boolean mIsReadyDelete = false; // Is checkbox checked
				...
				public void run() {
					...
					try {
						...
						if (headBean.mFullScreenIcon != null
								&& !headBean.mFullScreenIcon.equals("")) {
	...
	public void removeDeletedMsg(MessageHeadBean msg) {
		if (mMessageHeads != null) {
			mMessageHeads.remove(msg);
		}
	}

	/**
	 * 功能简述: 将消息列表封装到ArrayList<MessageHeadBean>中 功能详细描述:
	 * 将JSONArray的消息列表数据转换成ArrayList<MessageHeadBean> mMessageHeads 注意:
	 * 
	 * @param msgs
	 */
	public void praseMsgsHead(JSONArray msgs, List<JSONObject> themJsonObjects) {
		for (int i = 0; i < msgs.length(); i++) {
			...
			if (obj != null) {
				...
				if (type == MessageBaseBean.TYPE_SUPER_THEME) {
					if (themJsonObjects != null) {
						themJsonObjects.add(obj);
					}
					continue;
				}
				...
				if ((viewType & MessageBaseBean.VIEWTYPE_DESK_TOP) != 0) {
					...
					msg.mIsColsed = ConvertUtils.int2boolean(getJasonIntValue(obj,
							MessageBaseBean.TAG_MSG_ISCLOSED));
				}
				if (type == MessageBaseBean.TYPE_HTML) {
					msg.mUrl = getJasonStringValue(obj, MessageBaseBean.TAG_MSG_URL);
				}

				if (type == MessageBaseBean.TYPE_COUPON) {
					JSONArray array = null;
					try {

						array = new JSONArray(getJasonStringValue(obj,
								MessageBaseBean.TAG_MSG_COUPONLIST));
					} catch (Exception e) {
						// TODO: handle exception
					}
					for (int j = 0; array != null && j < array.length(); j++) {
						JSONObject coupon = (JSONObject) array.opt(j);
						if (coupon != null) {
							String code = getJasonStringValue(coupon,
									MessageBaseBean.TAG_MSG_COUPON_CODE);
							int value = getJasonIntValue(coupon,
									MessageBaseBean.TAG_MSG_COUPON_VALUE);
							String eTime = getJasonStringValue(coupon,
									MessageBaseBean.TAG_MSG_PROMOTION_ETIME);
							String sTime = getJasonStringValue(coupon,
									MessageBaseBean.TAG_MSG_PROMOTION_STIME);
							StringBuffer builder = new StringBuffer();
							builder.append(type).append("#").append(code).append("#")
									.append(value).append("#").append(sTime).append("#")
									.append(eTime);
							msg.mCoupon = builder.toString();
						}
					}
				} else if (type == MessageBaseBean.TYPE_PROMOTION) {
					JSONObject promotionJson = null;
					try {

//						array = new JSONArray(getJasonStringValue(obj,
//								MessageBaseBean.TAG_MSG_PROMOTIONLIST));
					    promotionJson = new JSONObject(getJasonStringValue(obj,
                                MessageBaseBean.TAG_MSG_PROMOTIONLIST));
					} catch (Exception e) {
						// TODO: handle exception
					}
					...
					if (promotionJson != null) {
					    String code = getJasonStringValue(promotionJson,
					            MessageBaseBean.TAG_MSG_PROMOTION_CODE);
					    String sTime = getJasonStringValue(promotionJson,
					            MessageBaseBean.TAG_MSG_NEW_PROMOTION_STIME);
					    String eTime = getJasonStringValue(promotionJson,
					            MessageBaseBean.TAG_MSG_NEW_PROMOTION_ETIME);
					    StringBuffer builder = new StringBuffer();
					    builder.append(type).append("#").append(code).append("#")
					    .append(sTime).append("#").append(eTime);
					    msg.mPromotion = builder.toString();
					}
...
//						}
//					}
				} else if (type == MessageBaseBean.TYPE_INSTRUCT) {
					msg.mInstruct = getJasonStringValue(obj, MessageBaseBean.TAG_MSG_INSTRUCT);
				}
				...
				if ((viewType & MessageBaseBean.VIEWTYPE_STATUS_BAR) != 0) {
					...
					msg.mFullScreenIcon = getJasonStringValue(obj,
							MessageBaseBean.TAG_MSG_FULL_SCREEN_ICON);
				}
				...
				msg.mIsWifi = getJasonIntValue(obj, MessageBaseBean.TAG_MSG_IS_WIFI);
				msg.mPackageName = getJasonStringValue(obj, MessageBaseBean.TAG_MSG_PACKAGENAME);
				msg.mMapId = getJasonStringValue(obj, MessageBaseBean.TAG_MSG_MAPID);
				msg.mMsgOutDateTime = getJasonStringValue(obj, MessageBaseBean.TAG_MSG_OUT_DATE);
				msg.mAddact = getJasonIntValue(obj, MessageBaseBean.TAG_MSG_ADDACT);
				msg.mIsadv = getJasonIntValue(obj, MessageBaseBean.TAG_MSG_ISADV);
				msg.mMsgimgurl = getJasonStringValue(obj, MessageBaseBean.TAG_MSG_IMGURL);
