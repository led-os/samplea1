...
public class DeliverMsgManager {
	...
	private HashMap<Long, INotifyHandler> mHandlerMap = new HashMap<Long, INotifyHandler>();
	 ...
	 * @return
	 */
	public synchronized void registerMsgHandler(long handlerID, INotifyHandler handler) {
	...
	public synchronized void unregisterMsgHandler(long handlerID) {
		INotifyHandler handler = mHandlerMap.remove(handlerID);
		// if (handler == null)
		// {
		// try
		// {
	...
	public void onChange(long handlerID, int msgID, Object obj) {
		INotifyHandler handler = mHandlerMap.get(handlerID);
		if (handler != null) {
	 ...
	 *            消息处理器
	 */
	public synchronized void registerDispenseMsgHandler(int startId, int endId, INotifyHandler handler) {
	 ...
	 *            消息处理器
	 */
	public synchronized void registerDispenseMsgHandler(Point point, INotifyHandler handler) {
	 ...
	 *            消息处理器
	 */
	public synchronized void registerDispenseMsgHandler(int handlerID, INotifyHandler handler) {
	...
	public class DispenseMsgHandler {
		Point mPoint;
		INotifyHandler mHandler;

		public DispenseMsgHandler(Point point, INotifyHandler handler) {
			mPoint = point;
			mHandler = handler;
		}
	}
