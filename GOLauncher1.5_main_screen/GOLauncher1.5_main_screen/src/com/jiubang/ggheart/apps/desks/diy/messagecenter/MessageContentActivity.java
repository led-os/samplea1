...
//	private final static int GET_MSG_ERRO = 0x04;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mManager = MessageManager.getMessageManager(ApplicationProxy.getContext());
		...
		if (bundle != null) {
			...
			isWifi = bundle.getInt("msgiswifi", 0);
		}
		if (id == null || msgUrl == null) {
			finish();
			return;
		}
		if (isWifi == 1 && !mManager.isWifiConnected()) {
			Toast.makeText(this, R.string.msgcenter_suggest_open_wifi,
					2000).show();
			finish();
			return;
		}
//		mDownloadController = DownloadControllerProxy.getInstance().getDownloadController();
...
//			mManager.updateStatisticsData(statisticsMsgs, fromWhere, 0, IPreferencesIds.SHAREDPREFERENCES_MSG_CLICK_TIMES, null);
			String mapId = msgHeadBean.mMapId == null ? "" : msgHeadBean.mMapId;
			mManager.saveStatisticsDatas(null, msgHeadBean.mId, mapId, "click", "1", fromWhere);
			mManager.updateStatisticsData(0);
		}
		
		//设置消息标题
		TextView title = (TextView) findViewById(R.id.message_center_topbar_title);
		title.setText(getResources().getString(R.string.message_center_detail_title));
	...
	private void findViewByIds() {
		...
		RelativeLayout backbtnArea = (RelativeLayout) findViewById(R.id.message_center_topbar_back_layout);
