...
public class SunSetHandler extends AbsFilterHandler {
	...
	private static final String SUNSET_BLACKBOARD1024_FILE_NAME = "blackboard.png";
	private static final String SUNSET_OVERLAY_MAP_FILE_NAME = "overlay_map.png";
	private static final String SUNSET_MAP_FILE_NAME = "sunset_map.png";
	public static final String[] EXSIT_FILE_ARRAY = new String[] {
		SUNSET_BLACKBOARD1024_FILE_NAME, SUNSET_OVERLAY_MAP_FILE_NAME,
		SUNSET_MAP_FILE_NAME };
	...
	public SunSetHandler(Context context) {
		try {
			...
			Bitmap input2Bitmap = BitmapFactory.decodeStream(context
					.getAssets().open(SUNSET_BLACKBOARD1024_FILE_NAME));
			Bitmap input3Bitmap = BitmapFactory.decodeStream(context
					.getAssets().open(SUNSET_OVERLAY_MAP_FILE_NAME));
			Bitmap input4Bitmap = BitmapFactory.decodeStream(context
					.getAssets().open(SUNSET_MAP_FILE_NAME));
		...
		if (info == null) {
			...
			try {
				bitmap = BitmapFactory.decodeStream(context.getAssets().open(
						SUNSET_BLACKBOARD1024_FILE_NAME));
			} catch (Exception e) {
				e.printStackTrace();
				return null;
			} catch (OutOfMemoryError e) {
	...
	public int getHadlerIds() {
		return IFilterTypeIds.FILTER_TYPE_SUNSET;
	}
