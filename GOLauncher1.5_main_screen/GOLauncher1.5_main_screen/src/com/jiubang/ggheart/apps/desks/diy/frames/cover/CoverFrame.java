...
public class CoverFrame extends FrameLayout {
    ...
    public static final String METHOD_ONCONFIGURATION_CHANGE = "onConfigurationChange";
	...
	public static final int COVER_VIEW_APPDRAWER_GUIDE = 0x7; // 功能提示的view
	
	public static final int COVER_VIEW_SCREEN_PREVIEW_GUIDE = 0x8; // 功能提示的view
	...
	public void init() {
		mMonitor = new CoverMonitor(mContext);
		mControl = new CoverFrameControl();
		mControl.setContainer(this);
	}
	...
	public boolean addCoverView(View view, int viewId) {
		...
		if (viewId != COVER_VIEW_APPDRAWER_GUIDE) {
			if (ShellPluginFactory.getShellManager().isViewVisible(IViewId.APP_DRAWER)) {
		...
		case ICoverFrameMsgId.COVER_FRAME_ADD_VIEW:
			...
			case COVER_VIEW_MESSAGECENTER:
				if (object != null && object instanceof View) {
					CoverView guideView = mControl.getCoverView(COVER_VIEW_SCREEN_GUIDE);
					if (guideView != null) {
						mControl.removeCoverView(COVER_VIEW_SCREEN_GUIDE);
					}
			...
			case COVER_VIEW_SCREEN_GUIDE:
				if (object != null && object instanceof View) {
					CoverView centerMessView = mControl.getCoverView(COVER_VIEW_MESSAGECENTER);
					if (centerMessView == null) {
						addCoverView((View) object, viewID);
					}
				}
				break;	
...
//					} // end if holidayView
			    ...
			    break;
			case COVER_VIEW_APPDRAWER_GUIDE :
			case COVER_VIEW_SCREEN_PREVIEW_GUIDE :
				if (object != null && object instanceof View) {
					addCoverView((View) object, viewID);
				}
				break;
	...
	public boolean createAppView(String packName) {
		try {
			View mainView = null;
			Context remoteContext = mContext.createPackageContext(packName,
					Context.CONTEXT_IGNORE_SECURITY | Context.CONTEXT_INCLUDE_CODE);
			if (packName.equals(CHRISTMAS2_PACKAGE_NAME) && Machine.IS_ICS) {
				mainView = createSuperThemeChristmas(remoteContext);
			} else {
				final ThemeInfoBean themeInfoBean = ThemeManager.getInstance(mContext).getCurThemeInfoBean();
				...
				if (Machine.IS_ICS && classDexNames != null && classDexNames.length > 0) {
					...
					mLoadDexUtil = LoadDexUtil.getInstance(mContext);
				...
				if (mainView == null) { // 免费主题的处理
					...
					final int resourceId = resources.getIdentifier("cover_root_view", "layout",
							packName);
					mainView = inflater.inflate(resourceId, null, false);
				}
			}
...
//		}
	...
	public void onConfigurationChange(Configuration newConfig) {
		doThemeViewMethod(METHOD_ONCONFIGURATION_CHANGE, newConfig, Configuration.class);
	}
