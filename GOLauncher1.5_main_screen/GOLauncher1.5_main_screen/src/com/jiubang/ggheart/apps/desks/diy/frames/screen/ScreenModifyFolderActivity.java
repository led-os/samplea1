	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mFinishButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// 按下完成按钮的话就不需要删除
				mDeleteInfo = false;
				String name = (null != mName.getText()) ? mName.getText().toString() : null;
				handleElments(name);
			}
		});
		...
		if (mAddAppToDockUnfit || mAddAppToDockFit
		...
		mInitThread = new Thread(ThreadName.INIT_MODIFY_FOLDER_APP_LIST) {
			...
			public void run() {
				synchronized (mMutex) {
					...
					if (!mAddAppToDockUnfit && !mAddAppToDockFit && !mGestureForApp) {
						// 获取文件夹的资源列表
				...
				switch (msg.what) {
					...
					case MARKITEMCHANGED : {
						...
						if ((mIsCreateFolder && mCheckedNum <= 1)
								|| (mAddAppToDockFit && mCheckedNum < 1)) {
							mFinishButton.setEnabled(false);
							mFinishButton.setTextColor(0XFFB9B9B9);
						} else {
