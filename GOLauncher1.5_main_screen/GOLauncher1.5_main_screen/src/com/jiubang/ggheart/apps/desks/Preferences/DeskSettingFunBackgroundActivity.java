...
//		info.setEntries(entries);
	...
	public void updateAppdrawerWallpaper() {
		...
		if (setting == FunAppSetting.BG_DEFAULT) {
			AppFuncThemeController mThemeCtrl = AppFuncThemeController.getInstance(
					ApplicationProxy.getContext());
			if (mThemeCtrl.isDefaultTheme()) { // 当前默认主题拿壁纸
...
//		mAppdrawerSelectCard.dismissDialog();
	...
	class GetAllThemesTask extends AsyncTask<Void, Void, String> {
		...
		protected void onPreExecute() {
			// 显示扫描等待提示框
			showDialog(DeskSettingBaseActivity.DIALOG_ID_INIT_LIST);
		}
		...
		protected void onPostExecute(String result) {
			// 1:取消扫描提示框
			removeDialog(DeskSettingBaseActivity.DIALOG_ID_INIT_LIST);
