...
public class DeskSettingVisualBackgroundTabView extends
	...
	private LinearLayout mWallPaperPicLinearLayout = null;
	private LinearLayout mDockPicLinearLayout = null;
	private LinearLayout mAppdrawerWallpaperPicLinearLayout = null;
	private DeskSettingItemBaseView mFilterWallpaper;
	private DeskSettingItemCheckBoxView mWallpaperScrollable;
	...
	protected void findView() {
		...
		mWallPaperPicLinearLayout = (LinearLayout) findViewById(R.id.wallpaperpicfather);
		mWallPaperPicLinearLayout.setOnClickListener(this);
		mDockPicLinearLayout = (LinearLayout) findViewById(R.id.dockpicfather);
		mDockPicLinearLayout.setOnClickListener(this);
		mAppdrawerWallpaperPicLinearLayout = (LinearLayout) findViewById(R.id.appdrawwallpaperpicfather);
		mAppdrawerWallpaperPicLinearLayout.setOnClickListener(this);

		mFilterWallpaper = (DeskSettingItemBaseView) findViewById(R.id.filterWallpaper);
		mFilterWallpaper.setOnClickListener(this);
		...
		// mAppdrawerSelectCard = (DeskSettingItemListView)
		// findViewById(R.id.appdrawer_selectcard);
		// mAppdrawerSelectCard.setOnValueChangeListener(this);

		mWallpaperScrollable = (DeskSettingItemCheckBoxView) findViewById(R.id.wallpaper_scrollable);
		mWallpaperScrollable.setOnValueChangeListener(this);
	...
	private void initAppDrawTabBgList() {
		...
		// DeskSettingSingleInfo info =
		// mAppdrawerSelectCard.getDeskSettingInfo()
		// .getSingleInfo();
	...
	public void load() {
		...
		if (FunctionPurchaseManager.getInstance(getContext()).getPayFunctionState(
				FunctionPurchaseManager.PURCHASE_ITEM_FILTER) == FunctionPurchaseManager.STATE_CAN_USE) {
			mFilterWallpaper.setImagePrimeVisibile(View.GONE);
		} else {
			mFilterWallpaper.setImagePrimeVisibile(View.VISIBLE);
		}
	}
	...
	public void save() {
		...
		if (newstylevalue == DOCK_BG_NON) {
			...
			SettingProxy.updateShortCutCustomBg(true);
	...
	public void onClick(View v) {
		if (v == mWallpaperTitle || v == mWallpaperPic
				|| v == mWallpaperBaseLine || v == mWallPaperPicLinearLayout) {
			// 处理快速点击的BUG
			...
			mDialog.setOnItemClickListener(new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int item) {
					...
					if (item == 0) {
						...
						GuiThemeStatistics
								.goLauncherUserBehaviorStaticDataCache(IGoLauncherUserBehaviorStatic.PREFERENCES_BG_GO);
					} else {
						GuiThemeStatistics
								.goLauncherUserBehaviorStaticDataCache(IGoLauncherUserBehaviorStatic.PREFERENCES_BG_CUSTOM);
			...
			mAppdrawerBgSettingValue = mAppdrawerListView.getDeskSettingInfo()
					.getSingleInfo().getSelectValue();
		} else if (v == mFilterWallpaper) {
			mFilterWallpaper.setImageNewVisibile(View.GONE);
			if (WallpaperControler.getInstance().isLiveWallpaper()) {
				Toast.makeText(
						getContext(),
						getContext().getString(
								R.string.wallpaper_filter_cannot_use),
						Toast.LENGTH_LONG).show();
			} else {
				Intent intent = new Intent(getContext(), FilterActivity.class);
				intent.putExtra(FilterActivity.PAY_ENTRANCE_ID_INDEX, 1);
	...
	public boolean onValueChange(DeskSettingItemBaseView baseView,
		...
		} else if (baseView == mWallpaperScrollable) {
			mWallpaperScrollable.setIsCheck((Boolean) newValue);
			int summaryId = ((Boolean) newValue) ? R.string.normal_wallpaper
					: R.string.haploid_wallpaper;
			mWallpaperScrollable.setSummaryText(summaryId);
			if (null != mScreenInfo) {
				if (mScreenInfo.mWallpaperScroll != mWallpaperScrollable
						.getIsCheck()) {
					mScreenInfo.mWallpaperScroll = mWallpaperScrollable
							.getIsCheck();
					SettingProxy
							.updateScreenSettingInfo(mScreenInfo);
	...
	private void initSaveFile() {
		...
		if (null == themeBean) {
			// 默认主题
			themeName = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
		} else {
			// 其他主题
	...
	public void updateAppdrawerWallpaper() {
		...
		if (setting == FunAppSetting.BG_DEFAULT) {
			AppFuncThemeController themeCtrl = AppFuncThemeController
					.getInstance(getContext());
			if (themeCtrl.isDefaultTheme() == false) {
				try {
					drawable = (BitmapDrawable) themeCtrl.getDrawable(themeCtrl
							.getThemeBean().mWallpaperBean.mImagePath);

					// 重新生成一张缩小图，不直接用功能表的图是因为功能表可能会对这张图recycle
					int height = mAppdrawerWallpaperPic.getHeight();
					int width = (int) (drawable.getIntrinsicWidth() * 1f / drawable
							.getIntrinsicHeight()) * height;
					drawable = BitmapUtility.zoomDrawable(getContext(),
		...
		if (setting == FunAppSetting.BG_DEFAULT) {
			...
			if (themeCtrl.isDefaultTheme() == false) {
				try {
					...
					// 重新生成一张缩小图，不直接用功能表的图是因为功能表可能会对这张图recycle
					showHeight = mAppdrawerWallpaperPic.getHeight();
					showWidth = (int) (drawable.getIntrinsicWidth() * 1f / drawable
							.getIntrinsicHeight()) * showHeight;
					drawable = BitmapUtility.zoomDrawable(getContext(),
							drawable, showWidth, showHeight);
				} catch (Exception e) {
					// 在某些主题使用了9切图作为背景，导致程序崩溃
					e.printStackTrace();
				} catch (OutOfMemoryError e) {
				...
				drawable = mWallpaperPic.getDrawable();
				showWidth = getPicShowWidth(drawable, showHeight);
			}
	...
	public void setInfos(ShortCutSettingInfo shortCutInfo,
		...
		if (null != mScreenInfo) {
			...
			mWallpaperScrollable.setIsCheck(scrollable);
	...
	public void checkNewMark(DeskSettingNewMarkManager newMarkManager) {
		if (newMarkManager.isShowNew(NewMarkKeys.WALLPAPER_FILTER, true)) {
			mFilterWallpaper.setImageNewVisibile(View.VISIBLE);
		}
	}
