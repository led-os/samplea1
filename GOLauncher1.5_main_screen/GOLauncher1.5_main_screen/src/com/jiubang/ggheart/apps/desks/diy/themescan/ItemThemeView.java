...
public class ItemThemeView extends LinearLayout implements BroadCasterObserver {
	...
	private DeskTextView mTextView; // 主题应用类型（如应用、下载、免费、付费等）
	private DeskTextView mInsideTextView; // 名称
	...
	public void setThemeData(ThemeInfoBean bean, int position) {
		...
		if (bean.getBeanType() == ThemeConstants.THEMEM_COUPON_ID) {
			drawable = getContext().getResources().getDrawable(R.drawable.mycoupons);
			...
			drawable = themeImageManager.getImageByThemeInfo(mThemeData, this);
			...
			if (mThemeData.getPackageName() != null
					&& (mThemeData.getPackageName().equals(
							IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3_NEWER) || mThemeData
							.getPackageName().equals(IGoLauncherClassName.DEFAULT_THEME_PACKAGE_3))) {
	...
	protected void onFinishInflate() {
		...
		mTextView = (DeskTextView) findViewById(R.id.imgbtn_get);
		mInsideTextView = (DeskTextView) findViewById(R.id.insidename);
	...
	@Override
	public void onBCChange(int msgId, int param, final Object ...object) {
		switch (msgId) {
			case ThemeImageManager.FINISH_LOAD_IMAGE : {
				if (mImageView != null && object != null && object[0] instanceof Drawable) {
					mImageView.setImageDrawable((Drawable) object[0]);
				}
			}
				break;
			...
			case ThemeImageManager.EVENT_NETWORK_ICON_CHANGE : {
				...
				if (mImageView != null
						&& object != null
						&& object[0] instanceof BitmapDrawable
						&& (object[1] != null && mThemeData.getFirstPreViewDrawableName() != null && mThemeData
								.getFirstPreViewDrawableName().endsWith((String) object[1]))) {
					mImageView.setImageDrawable((BitmapDrawable) object[0]);
				}
			}
				break;
