...
public class DockAddIconFrame extends AbstractFrame implements OnAddIconClickListner {
	...
	public Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == MESSAGE_CLEAR_DOCK_RED_BG) {
				MsgMgrProxy.sendMessage(this, IDiyFrameIds.SHELL_FRAME, IDockMsgId.DOCK_ADD_ICON_RED_BG, -1,
						null, null);
			}
		}
	};
	...
	private void setDockHeight() {
		int extraHeight = GoLauncherActivityProxy.isPortait()
				? DrawUtils.getNavBarHeight()
				: DrawUtils.getNavBarWidth();
		mDockHeight = DockUtil.getBgHeight() + extraHeight;
	}
	...
	public boolean handleMessage(Object who, int msgId, int param, Object ...obj) {
		...
		switch (msgId) {
			//dock添加图标初始化
			case IDockMsgId.DOCK_ADD_ICON_INIT :
				mBlankIndexInRow = param;
				mCurDockSize = (Integer) obj[0];
				mAddMaxSize = DockUtil.ICON_COUNT_IN_A_ROW - mCurDockSize;
				break;

			//上一次的添加图标完成
			case IDockMsgId.DOCK_ADD_ICON_ADD_FINISH:
				//添加完后修改添加位置等信息
			...
			//监听HOME键
			case ICommonMsgId.BACK_TO_MAIN_SCREEN:	 //go桌面作为默认桌面
			case IFrameworkMsgId.SYSTEM_ON_NEW_INTENT :	 //点击HOME键选择go桌面
				onDestory();	
				break;
	...
	public void selectIconOfItem(int type, View view, int position, Object infos) {
		...
		int screenWidth = GoLauncherActivityProxy.getScreenWidth();	//屏幕宽度
		int screenHeight = GoLauncherActivityProxy.getScreenHeight();	//屏幕高度

		//判断通知栏是否隐藏，没有隐藏就减去通知栏高度
		// 保持全屏高度（包含通知栏高度），解决自动飞入Dock栏动画卡顿问题（计算值与实际值相差一个状态栏高度）
//		if (!mIsHideStatusbar) {
...
//		}
		...
		// Dock层添加图标
		MsgMgrProxy.sendMessage(this, IDiyFrameIds.DOCK, IDockMsgId.DOCK_ADD_ICON_ADD_ONE, mBlankIndexInRow,
				infos, null);
	}
	...
	public void setDockRedBg() {
		...
		MsgMgrProxy.sendMessage(this, IDiyFrameIds.SHELL_FRAME, IDockMsgId.DOCK_ADD_ICON_RED_BG, 1, null, null);
