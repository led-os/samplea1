...
//	public final static int RESTOREDEFAULT = 6;
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.backup_desk_item :
				if (SettingProxy.getDeskLockSettingInfo().mRestoreSetting) {
					InvokeLockControler.getInstance(getApplicationContext()).startLockAction(
							InvokeLockControler.ACTION_ID_RESTORDB, new ActionResultCallBack() {

								@Override
								public void onUnlockSuccess(int actionId) {
									// TODO Auto-generated method stub
									StatisticsData.saveUseRecordPreferences(
											getApplicationContext(),
											StatisticsData.BACKUP_ITEM); //统计
									showBackupDialog();
								}
								@Override
							...
							}, DeskSettingBackupActivity.this, DeskSettingBackupActivity.this.getString(R.string.backup_desk));
				} else {
					...
					InvokeLockControler.getInstance(getApplicationContext()).startLockAction(
							InvokeLockControler.ACTION_ID_RESTORDB, new ActionResultCallBack() {
								...
								public void onUnlockSuccess(int actionId) {
									...
									StatisticsData.saveUseRecordPreferences(
											getApplicationContext(),
											StatisticsData.RESETBACKUP_ITEM); //统计
									showRestoreDBDialog();
								}
							...
							}, DeskSettingBackupActivity.this, DeskSettingBackupActivity.this.getString(R.string.restore_db_title));
				} else {
					...
					InvokeLockControler.getInstance(getApplicationContext()).startLockAction(
							InvokeLockControler.ACTION_ID_RESTORDB, new ActionResultCallBack() {
								...
								public void onUnlockSuccess(int actionId) {
									...
									StatisticsData.saveUseRecordPreferences(
											getApplicationContext(),
											StatisticsData.RESETDEFAULT_ITEM); //统计
							...
							}, DeskSettingBackupActivity.this, DeskSettingBackupActivity.this.getString(R.string.reset_to_default_title));
				} else {
					StatisticsData.saveUseRecordPreferences(this,
							StatisticsData.RESETDEFAULT_ITEM); //统计
					showResetDefaultDialog();
			...
			case R.id.more_go_backup_item :
				...
				GoAppUtils.gotoGobackup(this); // GO备份接入
				break;
			default :
				break;
		}
	...
	private synchronized void showBackupDialog() {
		...
		mNormalDialog.show();
		boolean isBackuped = isBackuped();
		int summary;
		if (!isBackuped) {
			summary = R.string.backup_db_summary;
		} else {
			summary = R.string.backuped_db_summary;
		}
		mNormalDialog.setTitle(this.getString(R.string.attention_title));
		mNormalDialog.setMessage(this.getString(summary));
		View.OnClickListener listener = new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (!Environment.MEDIA_MOUNTED.equals(Environment
					...
					DeskToast.makeText(DeskSettingBackupActivity.this, msg,
					...
					StatisticsData.saveStringData(
							DeskSettingBackupActivity.this,
							IPreferencesIds.BACKUP, "1"); //统计
					StatisticsData
							.backUpLanguageSetting(DeskSettingBackupActivity.this);
				}
			}
		};
		if (isBackuped) {
			mNormalDialog.setPositiveButton(null, listener);
		} else {
			mNormalDialog.setPositiveButton(
					R.string.backup_db_dialog_button_ok, listener);
	}
	}
...
//					new ImportDatabaseTask().execute();
					...
					task.setActivity(DeskSettingBackupActivity.this);
					task.setListner(DeskSettingBackupActivity.this);
					task.setType(ExportDatabaseTask.TYPE_LOCAL);
					task.execute();
				}
			}
		});
	...
	private class RestoreDefaultTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				PrivatePreference.getPreference(DeskSettingBackupActivity.this).restoreDefault();
...
//	}
	...
	public void onImportPostExecute(int type, String msg) {
		PrivatePreference pref = PrivatePreference.getPreference(this);
		pref.putBoolean(PrefConst.KEY_HAS_RESTORE_FLAG, true);
		pref.commit();
	}
