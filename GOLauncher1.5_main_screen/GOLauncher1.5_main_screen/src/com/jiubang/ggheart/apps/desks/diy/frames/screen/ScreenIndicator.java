...
public class ScreenIndicator extends Indicator {
	...
	public ScreenIndicator(Context context, AttributeSet att, int defStyle) {
		...
		a.recycle();
		sShowmode = GoContentProviderUtil.getIndicatorStyle(getContext());
	...
	public void setCurrent(int current) {
		...
		if (0 <= mCurrent && mCurrent < childcount) {
			((ImageView) getChildAt(mCurrent)).setImageDrawable(unfocus != null
					? unfocus
					: mUnfocus);
		}
		if (0 <= current && current < childcount) {
			((ImageView) getChildAt(current)).setImageDrawable(focus != null ? focus : mFocus);
			mCurrent = current;
		}
	}
