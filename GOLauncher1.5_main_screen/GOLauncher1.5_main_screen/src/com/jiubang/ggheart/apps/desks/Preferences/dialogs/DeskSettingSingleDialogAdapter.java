...
public class DeskSettingSingleDialogAdapter extends BaseAdapter {
	...
	private CharSequence[] mValues; //值
	private CharSequence[] mNeedPayList; //付费功能
	...
	public DeskSettingSingleDialogAdapter(Context context,
		...
		mValues = deskSettingSingleInfo.getEntryValues();
		mNeedPayList = deskSettingSingleInfo.getNeedPayList();
		mNeedShowNewTag = deskSettingSingleInfo.getNeedShowNewTag();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		final RadioButton singleChoiceButton = (RadioButton) convertView.findViewById(R.id.desk_setting_dialog_item_radiobtn);
		...
		//是否需要付费标志
		ImageView primeImage = (ImageView) convertView.findViewById(R.id.desk_setting_dialog_item_prime_image);
		ImageView newImage = (ImageView) convertView.findViewById(R.id.new_image);
		newImage.setVisibility(View.GONE);
		primeImage.setVisibility(View.GONE);
		if (mValues != null && mValues.length > 0 && position < mValues.length && mNeedPayList != null && mNeedPayList.length > 0) {
			int needPaySize = mNeedPayList.length;
			for (int i = 0; i < needPaySize; i++) {
				if (mNeedPayList[i].equals(mValues[position])) {
					primeImage.setVisibility(View.VISIBLE);
				}
			}
		}
		if (mValues != null && mValues.length > 0 && position < mValues.length && mNeedShowNewTag != null && mNeedShowNewTag.length > 0) {
			int needPaySize = mNeedShowNewTag.length;
			for (int i = 0; i < needPaySize; i++) {
				if (mNeedShowNewTag[i].equals(mValues[position])) {
