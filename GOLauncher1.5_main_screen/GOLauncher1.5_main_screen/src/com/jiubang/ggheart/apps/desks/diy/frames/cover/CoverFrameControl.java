...
//			}
	...
	public synchronized boolean addCoverView(CoverView view, int index, int visibility) {
		...
		final int realIndex = index + 1; // 因为实际到个数要加上桌面到DiyFrame这一层，所以相应到child个数也+1
		final int size = mCoverViews.size();
		if (realIndex > size) {
			...
			mCoverParent.addView(view.getView(), Math.max(1, realIndex));
//			mCoverParent.addView(view.getView(), index + 1);
		}
		ret = true;
