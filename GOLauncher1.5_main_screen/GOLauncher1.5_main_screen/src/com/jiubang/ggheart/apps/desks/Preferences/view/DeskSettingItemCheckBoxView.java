...
public class DeskSettingItemCheckBoxView extends DeskSettingItemBaseView {
	...
	public void initCheckBox() {
		...
		if (mIsBtnCheckBox) {
			...
			buttonDrawable = R.drawable.desk_setting_checkbox_new;
		}
	...
	public void updateEnabledStatus(boolean enabled) {
		if (enabled) {
			if (!mIsBtnCheckBox) { // 是打钩类型的
				mCheckBox.setButtonDrawable(mButtonDrawable);
			}
		} else {
			if (!mIsBtnCheckBox) { // 是打钩类型的
				if (getIsCheck()) { // 且选中的才换图
					mCheckBox.setButtonDrawable(R.drawable.desk_setting_checkbox_checked_disable);
				}
			}
		}
	}
