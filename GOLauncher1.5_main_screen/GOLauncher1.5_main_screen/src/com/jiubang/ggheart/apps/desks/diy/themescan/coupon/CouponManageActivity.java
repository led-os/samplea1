...
public class CouponManageActivity extends Activity 
			implements OnClickListener, BroadCasterObserver,
			OnItemClickListener {
	public static final String NEED_SHOW_PURCHASE_DIALOG = "need_show_purchase_dialog";
	public static final String PURCHASE_ITME_ID = "purchase_item_id";
...
//	private static final String COUPON_PURCHASEID_GETJAR_099 = "android.test.purchased"; //使用0.99元
//	private static final String COUPON_PURCHASEID_GETJAR_199 = "android.test.purchased"; //使用1.99元
//	private static final String COUPON_PURCHASEID_GETJAR_299 = "android.test.purchased"; //使用2.99元
//	private static final String COUPON_PURCHASEID_GETJAR_399 = "android.test.purchased"; //使用3.99元
...
//		mThemeInfoBean = intent.getParcelableExtra(MyThemeReceiver.USE_COUPON_THEME_INFO);
	...
	private void init() {
		...
		mGridView = (GridView) findViewById(R.id.couponlist);
...
//		registerPurchaseReceiver();
	 ...
	 * 给字符串加上下划线
	 */
	private void spanEmptyText() {
		String text = mEmptyText.getText().toString();
		int facebookIndex = text.indexOf("Facebook");
//		int twitterIndex = text.indexOf("Twitter");
//		int googleIndex = text.indexOf("Google");
		SpannableString ss = new SpannableString(text);
		if (facebookIndex > 0) {
			ss.setSpan(new ClickableSpan() {
				@Override
				public void onClick(View widget) {
					goToSomewhere("com.facebook.katana", "https://www.facebook.com/");
				}
			}, facebookIndex, facebookIndex + 8, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
		
//		ss.setSpan(new ClickableSpan() {
//			@Override
//			public void onClick(View widget) {
//				goToSomewhere("com.twitter.android", "http://www.twitter.com/");
//			}
//		}, twitterIndex, twitterIndex + 7, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
//		
//		ss.setSpan(new ClickableSpan() {
//			@Override
//			public void onClick(View widget) {
//				goToSomewhere("com.google.android.apps.plus", "https://plus.google.com/");
//			}
//		}, googleIndex, googleIndex + 7, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		
		mEmptyText.setText(ss);
		mEmptyText.setMovementMethod(LinkMovementMethod.getInstance());
	}
	
	private void goToSomewhere(String pkgName, String httpUrl) {
		try {
			Intent intent = getPackageManager().getLaunchIntentForPackage(pkgName);
			startActivity(intent);
		} catch (Exception e) {
			GoAppUtils.gotoBrowserInRunTask(this, httpUrl);
		}
	}
...
//						String pkgName = intent.getStringExtra(ThemeAppInBillingManager.EXTRA_FOR_PACKAGENAME);
//						if (Consts.DEBUG) {
//							Log.i(Consts.TAG, "CouponMananerActivity  mPurchaseStateReceiver itemId:" + itemId + ",state:"
//									+ state);
//						}
//						if (pkgName.equals(mThemePkgName) && state == 1) {
//							mCacheManager.clearCache(mCouponCode);
//							GuiThemeStatistics.guiStaticData("-1", "j009", 1, "-1", "-1", "-1", "-1");
//							//付费成功，关掉页面
...
//								ThemePurchaseManager mPurchaseManager = ThemePurchaseManager.getInstance(getApplicationContext());
////								mPurchaseManager.purchaseState(state, pkgName);
//								if (mThemeInfoBean != null) {
//									if (Consts.DEBUG) {
//										Log.i(Consts.TAG, "普通主题.....下载应用, pkgName = " + mThemeInfoBean.getPackageName() + ",url = " + mThemeInfoBean.getDownLoadUrl()
//												+ ",themeName = " + mThemeInfoBean.getThemeName());
//									}
			...
			switch (msg.what) {
				case MSG_SCAN_COUPON_FINISH :
					...
					if (msg.obj != null && msg.obj instanceof ArrayList
						...
						mGridView.setOnItemClickListener(CouponManageActivity.this);
					} else {
				...
				case GET_COUPON_ERROR :
					Toast.makeText(CouponManageActivity.this, R.string.msgcenter_msg_update_erro,
							1000).show();
					break;
				case GET_COUPON_NO_NETWORK :
					Toast.makeText(CouponManageActivity.this, R.string.http_exception, 1000).show();
					break;
				case GET_COUPON_FINISH_FAIL :
					...
					Toast.makeText(CouponManageActivity.this, toastId, 1000).show();
					break;
				case GET_COUPON_FINISH_SUCCESS :
					CouponBean bean = (CouponBean) msg.obj;
					if (bean == null) {
						Toast.makeText(CouponManageActivity.this, R.string.coupon_get_fail_invalid,
								1000).show();
						return;
					}
					if (!FileUtil.isSDCardAvaiable()) {
						Toast.makeText(CouponManageActivity.this, R.string.coupon_click_get_fail,
								1000).show();
						return;
					}
					mEmptyText.setVisibility(View.GONE);
					mGridView.setVisibility(View.VISIBLE);
					Toast.makeText(CouponManageActivity.this, R.string.coupon_get_success, 1000)
							.show();
					if (mAdapter == null) {
						mAdapter = new CouponAdapter();
					}
	...
	public void onClick(View v) {
		...
		switch (id) {
			case R.id.back :
				finish();
				break;
			case R.id.coupon_themes:
//				Intent intent = new Intent();
//				intent.putExtra("ty", BannerDetailActivity.COUPON_SPEC_ID);
//				intent.putExtra("title", getString(R.string.coupon_themes));
//				intent.setClass(this, BannerDetailActivity.class);
//				intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//				startActivity(intent);
				GoAppUtils.goToThemeSpec(this, BannerDetailActivity.COUPON_SPEC_ID, false);
			default:
				break;
		}
	}
	...
	public void onAddCoupon() {
		...
		dialog.setPositiveButton(null, new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (couponId.equals("")) {
					Toast.makeText(CouponManageActivity.this, R.string.coupon_code_not_empty, 1000)
							.show();
				} else {
...
////		mCouponSelectDialog = null;
//		if (bean != null && bean.isVaild()) {
//			int faceValue = bean.getFaceValue();
//			GuiThemeStatistics.guiStaticData("-1", "j009", 0, "-1", "-1", "-1", "-1");
...
//					startIntent.putExtra(AppInBillingRequestReceiver.EXTRA_FOR_ITEMID, itemId);
//					startIntent.putExtra(AppInBillingRequestReceiver.EXTRA_FOR_PACKAGENAME,
//							mThemePkgName);
//					startIntent.putExtra(AppInBillingRequestReceiver.EXTRA_FOR_PRODUCTUNMANAGED,
//							true);
//					startIntent.putExtra(AppInBillingRequestReceiver.EXTRA_FOR_THEMETYPE,
//							mThemeType);
...
//				}
//			} else {
//				Toast.makeText(CouponManageActivity.this, R.string.coupon_higher_price, 800).show();
//			}
//		}
//	}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		if (mIsGettingCoupon || (position != mGridView.getAdapter().getCount() - 1)) {
			return;
		}
...
//						StringBuilder sb = new StringBuilder(ThemeAppInBillingManager.STATISTICS_DATA_URL);
//						sb.append(Machine.getAndroidId());
//						sb.append("&goid=");
//						sb.append(StatisticsManager.getGOID(CouponManageActivity.this));
//						if (Consts.DEBUG) {
...
//			String imei = Statistics.getVirtualIMEI(CouponManageActivity.this);
//			String country = RecommAppsUtils.local(CouponManageActivity.this);
//			stringBuffer.append(imei).append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING).append(country)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING).append(packageName)	
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING).append(paidType)   
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING).append(paidSucessed) 
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //获取金币方式
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //是否点击
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //tapjoy是否成功返回金币
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //是否安装桌面
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //是否激活
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //是否带桌面安装
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //价格
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //安装时间戳
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_DEFAULT_VALUE) //付费成功时间戳
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
//					.append(GoStorePhoneStateUtil.getUid(CouponManageActivity.this)) //渠道号
//					.append(ThemeAppInBillingManager.STATISTICS_DATA_SEPARATE_STRING)
...
//		//对所有的统计数据进行加密
//		if (statistics != null) {
//			statistics = CryptTool.encrypt(statistics, ThemeAppInBillingManager.STATISTICS_DATA_ENCRYPT_KEY);
//			try {
...
//		if (null == context
//				|| null == ThemeManager.getInstance(context)) {
//			return;
//		}
//
