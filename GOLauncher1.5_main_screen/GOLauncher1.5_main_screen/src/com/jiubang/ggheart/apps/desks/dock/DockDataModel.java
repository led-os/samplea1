...
public class DockDataModel extends DataModel {
	...
	private void convertCursorToShortCutItem(final Cursor cursor, ArrayList<DockItemInfo> itemInfos) {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				if (((itemInfo.mItemInfo.mItemType == IItemType.ITEM_TYPE_APPLICATION || itemInfo.mItemInfo.mItemType == IItemType.ITEM_TYPE_SHORTCUT) && null == ((ShortCutInfo) itemInfo.mItemInfo).mIntent)
					...
					itemInfo.mItemInfo.mFeatureIconPackage = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
				}
		...
		int iconsize = DockUtil.getIconSize(size);
		...
		for (int i = 0; i < 5; i++) {
			...
			switch (i) {
				case 0 :
					...
					((ShortCutInfo) info1.mItemInfo).mFeatureTitle = ApplicationProxy.getContext()
			...
			((ShortCutInfo) info1.mItemInfo).mFeatureIconPackage = IGoLauncherClassName.DEFAULT_THEME_PACKAGE;
	...
	private void insertRecommendShortCut(long time) {
		...
		if (infosize > 0) {
			for (int j = 1; j < DockUtil.TOTAL_ROWS; j++) {
				for (int k = 0; k < DockUtil.ICON_COUNT_IN_A_ROW; k++) {
					DockItemInfo info = initDockItemInfos.get(addCount);
					info.mItemInfo.mInScreenId = time + (j * DockUtil.ICON_COUNT_IN_A_ROW + k);
