 ...
 * @date  [2012-10-17]
 */
public class FuncSearchResultItem implements Comparable<FuncSearchResultItem> {

	private FunAppItemInfo mFunAppItemInfo;
	/**
	 * 本地类型
	 */
	...
	public static final byte ITEM_TYPE_CLEAR_HISTORY = 11;
	/**
	 * 本地资源类型才需要这个字段；
	 */
	...
	public FunAppItemInfo getFunAppItemInfo() {
		return mFunAppItemInfo;
	}

	public void setFunAppItemInfo(FunAppItemInfo mFunAppItemInfo) {
		this.mFunAppItemInfo = mFunAppItemInfo;
	}

	@Override
	public int compareTo(FuncSearchResultItem another) {
		// 考虑到搜索的元素比较规则相对简单，所以用Comparable接口
		int ret = 0;
		if (mMatchIndex > another.mMatchIndex) {
			ret = 1;
		} else if (mMatchIndex < another.mMatchIndex) {
			ret = -1;
		}
		return ret;
	}
