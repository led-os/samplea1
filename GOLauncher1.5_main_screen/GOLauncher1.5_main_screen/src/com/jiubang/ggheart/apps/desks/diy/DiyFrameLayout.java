...
public class DiyFrameLayout extends FrameLayout {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		int topId = MsgMgrProxy.getTopFrameId();
		View topFrame = MsgMgrProxy.getContentView(topId);
		// 如果topframe不透明，仅绘制topframe
		if (IDiyFrameIds.INVALID_FRAME != topId && MsgMgrProxy.isOpaque(topId)) {
			// 全屏且不透明
	...
	private void drawFrame(View child, Canvas canvas) {
		if (child != null) {
