...
public class AppfuncSearchController implements BroadCasterObserver {
	...
	private AppfuncSearchController(Context context, Handler handler) {
		...
		mSearchObserver = new AppfuncSearchObserver();
	 ...
	 * @date  [2012-10-17]
	 */
	private class AppfuncSearchObserver implements SearchObserver {

		@Override
		public void onSearchSupported(boolean supported, Object timeStamp) {
			if (!supported) {
				mHandler.sendEmptyMessage(AppFuncSearchConstants.MSG_SHOW_SEARCH_CONNECT_EXCEPTION);
			}
		}
		...
		public void onSearchStart(String searchKey, int type, int currentPage) {
			switch (type) {
				case AppfuncSearchEngine.SEARCH_TYPE_WEB :
					if (currentPage > 1) {
						mHandler.sendEmptyMessage(AppFuncSearchConstants.MSG_SHOW_GOSTORE_LOAD_MORE_PROGRESS);
					} else {
						mHandler.sendEmptyMessage(AppFuncSearchConstants.MSG_SHOW_GOSTORE_PROGRESS_BAR);
					}
					break;
				case AppfuncSearchEngine.SEARCH_TYPE_LOCAL :
					mHandler.sendEmptyMessage(AppFuncSearchConstants.MSG_SHOW_LOCAL_PROGRESS_BAR);
					break;
				default :
					break;
			}
		...
		public void onSearchFinsh(String searchKey, List<?> list, int type, int resultCount,
			...
			switch (type) {
				...
				case AppfuncSearchEngine.SEARCH_TYPE_WEB :
					...
					if (currentPage > 1) {
						msg = mHandler
								.obtainMessage(AppFuncSearchConstants.MSG_DATACHANGE_GOSTORE_ADAPTER);
					} else {
						msg = mHandler
								.obtainMessage(AppFuncSearchConstants.MSG_SET_GOSTORE_RESULT_ADAPTER);
					}
					msg.arg2 = mSearchEngine.getSearchId();
					break;
				case AppfuncSearchEngine.LOCAL_SEARCH_FINISH :
					msg = mHandler.obtainMessage(AppFuncSearchConstants.MSG_SET_LOCAL_RESULT_ADAPTER);
					break;
				default :
					break;
			}
		...
		public void onSearchWithoutData(String searchKey, int type, Object timestamp) {
			...
			switch (type) {
				case AppfuncSearchEngine.SEARCH_ALL_RESOURCE :
					msg = mHandler.obtainMessage(AppFuncSearchConstants.MSG_SHOW_LOCAL_NODATA_VIEW);
					msg.obj = mContext.getString(R.string.appfunc_search_tip_no_match_data);
					break;
				case AppfuncSearchEngine.SEARCH_TYPE_WEB :
					msg = mHandler.obtainMessage(AppFuncSearchConstants.MSG_SHOW_GOSTORE_NODATA_VIEW);
		...
		public void onSearchException(String searchKey, Object obj, int type) {
			switch (type) {
				case AppfuncSearchEngine.SEARCH_TYPE_WEB :
					Message msg = mHandler.obtainMessage(AppFuncSearchConstants.MSG_SHOW_GOSTORE_NODATA_VIEW);
					msg.obj = mContext.getString(R.string.appfunc_search_tip_no_match_data_web);
					mHandler.sendMessage(msg);
					mHandler.sendEmptyMessage(AppFuncSearchConstants.MSG_SHOW_SEARCH_CONNECT_EXCEPTION);
					break;
		...
		public void onHistoryChange(List<?> list, int historyType, boolean hasHistory,
			...
			switch (historyType) {
				case AppfuncSearchEngine.HISTORY_TYPE_LOCAL :
					if (hasHistory) {
						msg = mHandler.obtainMessage(AppFuncSearchConstants.MSG_SHOW_LOCAL_HISTORY);
						@SuppressWarnings("unchecked")
						ArrayList<FuncSearchResultItem> resultItems = (ArrayList<FuncSearchResultItem>) list;
						msg.obj = resultItems;
					} else {
				...
				case AppfuncSearchEngine.HISTORY_TYPE_WEB :
					if (hasHistory) {
						...
						msg = mHandler.obtainMessage(AppFuncSearchConstants.MSG_SHOW_GOSTORE_HISTORY);
						msg.obj = resultItems;
					} else {
						if (isNotifly) {
							msg = mHandler
									.obtainMessage(AppFuncSearchConstants.MSG_SHOW_GOSTORE_NODATA_VIEW);
							msg.obj = mContext.getString(R.string.appfunc_search_no_history);
						} else {
							msg = mHandler.obtainMessage(AppFuncSearchConstants.MSG_SHOW_KEY_WORDS);
						}
...
//							msg.what = AppFuncSearchConstants.MSG_UPDATE_KEYWORDS;
//							mHandler.sendMessage(msg);
...
//		}
	...
	public void onBCChange(int msgId, int param, Object ...obj) {
		...
		switch (msgId) {
			...
			case BC_MSG_APPFUNC_FRAME_END :
				...
				ResponseHandler.unregister(mSearchObserver);
				break;
			...
			case BC_MSG_SEARCH_LOACL_RESOURCE :
				...
				if (key == null || "".equals(key)) {
					Message message = mHandler
							.obtainMessage(AppFuncSearchConstants.MSG_SHOW_LOCAL_NODATA_VIEW);
					message.obj = mContext.getString(R.string.appfunc_search_tip_no_match_data);
					break;
				} else if (mLocalLastKeyWords.equalsIgnoreCase((String) obj[0])
						&& obj[1] != null) {
					break;
				}
				...
				if (mSearchEngine != null) {
					...
					mSearchEngine.searchLocalResource(key, (Boolean) obj[1],
			...
			case BC_MSG_SEARCH_WEB_APPS :
				...
				// 相同的搜索关键词,不重复搜索。
				else if (mLastKeyWords.equalsIgnoreCase((String) obj[0]) && obj[1] != null) {
					Message msg = mHandler
							.obtainMessage(AppFuncSearchConstants.MSG_SET_GOSTORE_RESULT_ADAPTER);
					msg.obj = obj[1];
					msg.arg1 = mGoStoreSearchCount;
					mHandler.sendMessage(msg);
					mLastKeyWords = (String) obj[0];
					break;
				}
				mLastKeyWords = (String) obj[0];
				if (mSearchEngine != null) {
					mSearchEngine.searchWebApps((String) obj[0], System.currentTimeMillis());
				}
				break;
			...
			case BC_MSG_SAVE_HISTORY :
				if (mSearchEngine != null) {
					mSearchEngine.putSearchKeyToSharedPreference((String) obj[0], param);
				}
				break;
			case BC_MSG_CLEAR_HISTORY :
				if (mSearchEngine != null) {
					mSearchEngine.clearSearchKeysSharedPreference(param, (Boolean) obj[1]);
				}
				break;
			...
			case BC_MSG_SEARCH_ENGINE_MEDIA_DATA_READY :
				Message msg = mHandler.obtainMessage(AppFuncSearchConstants.MSG_MEDIA_DATA_READY);
				mHandler.sendMessage(msg);
				break;
			default :
				break;
	...
	private void hideImp(int needRemove) {
		Message message = mHandler.obtainMessage(AppFuncSearchConstants.MSG_CHANGE_IM_STATE);
		message.obj = false;
