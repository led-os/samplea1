...
public class ImagePreviewData {
	...
	private void initmThemeitems() {
		// 获取除默认主题以外的所有主题
		mThemeitems = ThemeManager.getInstance(ApplicationProxy.getContext()).getAllThemeInfosWithoutDefaultTheme();
		if (mThemeitems == null) {
		...
		defualtItem.setPackageName(IGoLauncherClassName.DEFAULT_THEME_PACKAGE);
	...
	public void initData(String themePkg) {
		...
		if (IGoLauncherClassName.DEFAULT_THEME_PACKAGE.equals(themePkg)) {
			mResNames = CustomIconRes.getDefaultResList();
		} else {
			DrawResourceThemeBean themeBean = ThemeManager.getInstance(ApplicationProxy.getContext()).getThemeResrouceBean(
					themePkg);
			if (themeBean != null) {
	...
	private void loadFolderIcon() {
		...
		ArrayList<ThemeInfoBean> allThemeInfoBeans = ThemeManager.getInstance(ApplicationProxy.getContext())
				.getAllThemeInfosWithoutDefaultTheme();
		int count = allThemeInfoBeans.size();
		mFolderPackageList.add(IGoLauncherClassName.DEFAULT_THEME_PACKAGE); // 添加默认的主题包名
		mFolderResNameList.add(FolderConstant.DEFAULT_RAWICON_RES); // 添加默认主题的文件名
