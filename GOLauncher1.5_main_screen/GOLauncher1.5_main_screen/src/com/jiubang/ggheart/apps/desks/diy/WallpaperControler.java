...
public class WallpaperControler {
	...
	// 交由系统绘制的标识
	private boolean mSysDrawSupport;
	// 支持透明通知栏的标识
	...
	public Handler mHandler = new Handler() {
		...
		public void handleMessage(android.os.Message msg) {
			...
			case MSG_UPDATE_WALLPAPER_TYPE :
				setWindowBackground(mSysDrawSupport); // 更新壁纸
				break;
			...
			if (mTsbSupport || isLiveWallpaper) {
				...
				if (fromIntentReceiver || mWallpaperDrawable == null) {
					try {
						mWallpaperDrawable = null;
						mWallpaperDrawable = mWallPaperManager.getDrawable();
						Resources resources = ApplicationProxy.getContext().getResources();
						int screenWidth = resources.getDisplayMetrics().widthPixels;
						int screenHeight = getRealHeight();
	...
	private boolean preventAmplifyWallpaper(Resources resources, int screenWidth) {
		if (mWallpaperDrawable != null) {
			...
			if (mWallpaperDrawable.getIntrinsicWidth() >= screenWidth) {
				if (mLastWallPaperDrawableWidth == 0) {
				...
				} else if (!SettingProxy.getScreenSettingInfo().mWallpaperScroll
					...
					setWindowBackground(mSysDrawSupport);
	...
	private Drawable prepareOrientationChange(Drawable walllPaper) {
		...
		if (null != activity
			...
			int screenW = GoLauncherActivityProxy.getScreenWidth();
	...
	private void updateWallpaperOffset(View window, int nums, int scrollX, int range) {
		if (!mSysDrawSupport || window == null) {
			return;
		}
	...
	private void setSuperThemeWallpaper(Context context, String resName) {
		try {
			...
			if (infoBean != null) {
				...
				Resources resources = ApplicationProxy.getContext().getPackageManager()
	...
	public void setTransparentStatusbarSupport(boolean bool) {
		if (GoLauncherActivityProxy.getActivity() != null) {
			StatusBarHandler.setStatusBarTransparent(GoLauncherActivityProxy.getActivity().getWindow(), bool);
			if (!Machine.isSupportAPITransparentStatusBar()) {
				setTransparentStatusbar(bool);
			}
		}
	}
	
	private void setTransparentStatusbar(boolean bool){
		mTsbSupport = bool;
	...
	public static boolean adjustWallpaperDimension(WallpaperManager wallpaperManager) {
		...
		if (wallpaperManager == null) {
			wallpaperManager = WallpaperManager.getInstance(ApplicationProxy.getContext());
		}
		try {
	...
	public BitmapDrawable adjustToWallpaperDrawable(Resources resources, int resId) {
		OutOfMemoryHandler.handle();
		...
		try {
			...
			if (resources.getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
				...
				screenH = getRealHeight();
			} else {
				screenW = getRealHeight();
...
//		InputStream in = null;
	...
	public int getRealHeight() {
		return DrawUtils.getRealHeight();
	}
	
	public boolean isSysDrawSupport() {
		return mSysDrawSupport;
	}
}