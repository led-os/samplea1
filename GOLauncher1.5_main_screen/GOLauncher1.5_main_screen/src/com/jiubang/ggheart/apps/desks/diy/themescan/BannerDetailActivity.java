 ...
 * @date  [2012-10-18]
 */
public class BannerDetailActivity extends Activity implements OnClickListener {
	...
	private View mGoTOCouponView;
	public static final int COUPON_SPEC_ID = 425;
//	public static final int COUPON_SPEC_ID = 349;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		mGoTOCouponView = findViewById(R.id.goto_coupon);
		confirmOrientation();
		Intent it = getIntent();
		...
		if (mSpecId == COUPON_SPEC_ID) {
			mGoTOCouponView.setVisibility(View.VISIBLE);
			mGoTOCouponView.setOnClickListener(this);
		}
	...
	protected void onDestroy() {
		// TODO Auto-generated method stub
		super.onDestroy();
		 ThemeDataManager.getInstance(this).removeData(ThemeConstants.LAUNCHER_SPEC_THEME_ID);
...
//			mlistViewLayout.configView(mViewConfig);
		...
		new Thread() {
			...
			public void run() {
				...
				if (!new File(fileName).exists()) {
					while (!MessageHttp.downloadZipRes(url, SPEC_RES_PATH, mSpecId
							+ VIEW_CONFIG_POSTFIX)
							&& retryCount < 3) {
						try {
							retryCount++;
	...
	public void onClick(View v) {
		// TODO Auto-generated method stub
		switch (v.getId()) {
			case R.id.goto_coupon :
				Intent intent = new Intent(this, CouponManageActivity.class);
				intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				startActivity(intent);
				break;
