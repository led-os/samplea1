...
public class DownloadService extends Service {
	...
	//下载管理类
	private DownloadManager mDownloadManager = DownloadManager.getInstance(ApplicationProxy.getContext());

	//安装管理类
	private InstallManager mInstallManager = InstallManager.getInstance(ApplicationProxy.getContext());
	...
	private IDownloadService.Stub mBinder = new IDownloadService.Stub() {
		...
		public void stopDownloadById(long taskId) throws RemoteException {
			if (mDownloadManager == null) {
				mDownloadManager = DownloadManager.getInstance(ApplicationProxy.getContext());
			}
			...
			if (name != null && !name.trim().equals("")) {
				try {
					IAidlDownloadListener downloadListener = (IAidlDownloadListener) Class
							...
							.newInstance(ApplicationProxy.getContext());
					DownloadTask task = getDownloadTaskById(taskId);
