...
public class DownloadAsyncTask extends AsyncTask<Object, DownloadTask, Integer> {
	...
	private void downloadFile(String downloadUrl, String filePath) throws Exception {
		...
		try {
			...
			boolean isInNewwork = GoStorePhoneStateUtil.isNetWorkAvailable(ApplicationProxy.getContext());
			boolean isInWifi = GoStorePhoneStateUtil.isWifiEnable(ApplicationProxy.getContext());
	...
	private HttpURLConnection openConnection(String downloadUrl) {
		...
		try {
			...
			if (ThemeHttp.isCWWAPConnect(ApplicationProxy.getContext())
					&& GoStorePhoneStateUtil.getNetWorkType(ApplicationProxy.getContext()) != GoStorePhoneStateUtil.NETTYPE_UNICOM) {
				Proxy proxy = null;
				if (GoStorePhoneStateUtil.getNetWorkType(ApplicationProxy.getContext()) == GoStorePhoneStateUtil.NETTYPE_TELECOM) {
					...
					// 联想的3gwap经测试不需设置代理
					String host = ThemeHttp.getProxyHost(ApplicationProxy.getContext());
					int port = ThemeHttp.getProxyPort(ApplicationProxy.getContext());
