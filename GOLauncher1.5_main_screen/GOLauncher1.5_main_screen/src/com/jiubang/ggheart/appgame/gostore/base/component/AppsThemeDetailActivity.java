...
public class AppsThemeDetailActivity extends Activity implements IFinishNotice {
	...
	public final static int REQUEST_INTERSTITIAL = 1;
	public final static int REQUEST_RESOLUTION = 2;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (bundle != null) {
			...
			int statEntraceId = bundle.getInt(RealTimeStatisticsContants.ENTRANCE_KEY, -1);
			if (-1 != statEntraceId) {
				//入口统计id不为-1时，有入口值
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			try {
				if (mDownloadController != null) {
					...
					// 所以这里需要赋值
					DownloadControllerProxy.getInstance().setDownloadController(mDownloadController);
				...
				public void handleMessage(Message msg) {
					...
					switch (msg.what) {
								...
								if (mWholeView != null) {
									...
									if (detailStyle == BIG_DETAIL_STYLE) {
										...
										mThemeDetailContainer = new ThemeDetailContainer(mContext,
												mDownloadController);
										mThemeDetailContainer.setActivity(AppsThemeDetailActivity.this);
										...
										if (mAppsDetailContainer == null) {
											mAppsDetailContainer = new AppsDetailContainer(mContext,
													mDownloadController);
											mAppsDetailContainer.setActivity(AppsThemeDetailActivity.this);
						...
						case APPLY_THEME :
							...
							intentGoLauncher.setClassName(AppsThemeDetailActivity.this,
									IGoLauncherClassName.GOLAUNCHER_ACTIVITY);
		...
		builder.setPositiveButton(R.string.locker_low_verson_update,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						if (!GoAppUtils.gotoMarket(AppsThemeDetailActivity.this,
								MarketConstant.APP_DETAIL + PackageName.LOCKER_PACKAGE)) {
	...
	private void countDownloadClick(AppDetailInfoBean infoBean) {
		...
		if (mStartType == AppsDetail.START_TYPE_APPMANAGEMENT) {
			...
			AppManagementStatisticsUtil.getInstance().saveUpdataClick(mContext, infoBean.mPkgName,
					infoBean.mAppId, 1, infoBean.mIcbackUrl);
		...
		switch (requestCode) {
			case REQUEST_INTERSTITIAL :
				if (resultCode == GetjarConstants.RESULT_NO_AD_DISPLAYED) {
					if (mThemeDetailContainer != null) {
						mThemeDetailContainer.initAdFullView(false);
					}
				} else {
					if (mThemeDetailContainer != null) {
						mThemeDetailContainer.setHasShowAd();
					}
				}
				break;
		}
