...
public class GridViewContainer extends FrameLayout implements IContainer, IModeChangeListener {
	 ...
	 * 头部的广告位
	 */
	private AppGameAdBannerHeadView mBannerView = null;
	...
	protected void onFinishInflate() {
		...
		mAdView = new RelativeLayout(getContext());
		
		mBannerView = new AppGameAdBannerHeadView(getContext()); 
...
//		mListView.setAdapter(mAdapter);
	...
	public void updateContent(ClassificationDataBean bean, boolean isPrevLoadRefresh) {
		...
		if (!mHasSetAdapter) {
			mHasSetAdapter = true;
			if (bean.adBanner != null && mBannerView != null) {
				mBannerView.updateContent(bean.adBanner);
				mBannerView.setVisibility(VISIBLE);
				mHeadView.addView(mBannerView);
			}
			if (mAdView != null) {
				mHeadView.addView(mAdView);
			}
			// TODO
			if (mHeadView != null) {
...
//		initAdView();
	...
	private void initAdView() {
		GoAdView adView = GoStoreAdView.getAdView(
				(Activity) getContext(), AdConstanst.PRODUCT_ID_LAUNCHER_STORE,
				AdConstanst.POS_ID_LAUNCHER_STORE_LIST_BANNER, mAdView);
		if (adView != null) {
