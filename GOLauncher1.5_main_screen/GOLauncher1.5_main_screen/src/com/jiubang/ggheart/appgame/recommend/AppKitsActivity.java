	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (-1 != statEntraceId) {
			...
			StatisticsManager.getInstance(ApplicationProxy.getContext()).upLoadBasicOptionStaticData(
					...
					Statistics.getUid(this), String.valueOf(statEntraceId), null,
					-1);
			//保存当前的key值
			RealTimeStatisticsUtil.saveCurrnetEntrance(this, statEntraceId);
		}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (bean.mTitle != null && !bean.mTitle.equals("")) {
				...
				for (BoutiqueApp app : bean.mAppInfoList) {
					if (!GoAppUtils.isAppExist(mContext, app.info.packname)) {
	...
	public void onClick(View v) {
		if (v == mGoToCenterBtn) {
			...
			AppsManagementActivity.startAppCenter(this, MainViewGroup.ACCESS_FOR_RECOMMENDLIST,
					false, RealTimeStatisticsContants.AppgameEntrance.DESK_ICON);
			this.finish();
