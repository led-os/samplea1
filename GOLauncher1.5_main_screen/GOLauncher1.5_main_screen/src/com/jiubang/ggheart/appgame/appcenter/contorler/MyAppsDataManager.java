	...
	private void sortByLocation(ArrayList<AppInfo> allAppsDataList) {
		if (allAppsDataList != null) {
			...
			if (!internalApps.isEmpty()) {
				...
				SortHelper.doSort(internalApps, method);
				internalAppSize = internalApps.size();
			}

			if (!externalApps.isEmpty()) {
				CompareMethod<ITitleCompareable> method = new CompareTitleMethod();
				SortHelper.doSort(externalApps, method);
	...
	private void sortByType(ArrayList<AppInfo> allAppsDataList, int type) {
		...
		switch (type) {
			...
			case 2:
				...
				// 以下的方法会调用一次onSortedCompleted，最后又调用一次？
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (list.get(i).getTitle() == null
				...
				firstLetter = SortUtils
						.changeChineseToSpell(mContext, list.get(i).getTitle(), R.raw.unicode2pinyin)
