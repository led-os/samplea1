...
public class AppsManagementActivity extends Activity {
	...
	private static final int REQUEST_RESOLUTION = 4;
	/**
	...
	/**
	 * 反注册消息接收者 与{@link MsgMgrProxy.registMsgHandler(IMessageHandler, int)}配对使用
	 * 
	 * @param handler
	 * @param msgId
	 * @return
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 加载系统渠道配置信息
		GOLauncherConfig.getInstance(this).loadConfig();
...
//		GoMarketPublicUtil.getInstance(sContext);
		...
		int statEntraceId = intent.getIntExtra(RealTimeStatisticsContants.ENTRANCE_KEY, -1);
		if (-1 != statEntraceId) {
			//统计登录
			RealTimeStatisticsUtil.upLoadLoginOptionStaticData(this, statEntraceId);
		}
		...
		//add by zzf 
		//统计程序启动入口信息
//		StatisticsManager.getInstance(this).upLoadBasicOptionStaticData(
//				RealTimeStatisticsContants.FUN_ID_APPGAME, null,
//				RealTimeStatisticsContants.OPERATE_LOGIN, RealTimeStatisticsContants.OPERATE_SUCCESS,
//				Statistics.getUid(this), StattisticsConstants.ENTRANCE_FROM_FUNC, null, -1);
		// Home键跳转标识
...
//		android.os.Process.killProcess(android.os.Process.myPid());
		Intent admobBroadcast = new Intent(AdViewInfoReceiver.UPLOAD_ADINFO_ACTION);
		sendBroadcast(admobBroadcast);
		
		FunctionPurchaseManager.destory();
		ProcessKiller.killSelf(this, 1200);
	};
	 ...
	 *            是否展示启动loading页
	 * @param statEntranceId
	 * 		  统计入口的标识，用来实时统计
	 */
	public static void startAppCenter(final Context context, int entranceId,
			boolean showFrontCover, int statEntranceId) {
		if (context != null) {
			Thread thread = new Thread("saveEntryCount") {
				@Override
				public void run() {
					StatisticsData.saveEntryCount(context,
							StatisticsData.ENTRY_APPS,
			...
			intent.putExtra(
					...
					entranceId);
			intent.putExtra(
					AppsManagementConstants.APPS_MANAGEMENT_SHOW_FRONTCOVER,
					showFrontCover);
	 ...
	 */
	public static void startAppCenter(final Context context, int entranceId,
			boolean showFrontCover, int typeid, int statEntranceId) {
		startAppCenter(context, entranceId, showFrontCover, statEntranceId);
	}
	/**
	 * 进入应用中心
	 * 
	 ...
	 * @author xiedezhi
	 */
	public static void startTopic(Context context, int typeId, boolean returnToAppCenter, int statEntrance) {
		if (context != null) {
			Intent intent = new Intent(context, MoreAppsActivity.class);
			intent.putExtra(MoreAppsActivity.TOPIC_TYPEID_KEY, typeId);
			intent.putExtra(MoreAppsActivity.TOPIC_RETUREACTION_KEY, returnToAppCenter);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_CODE_FOR_INSTALL) {
			...
			uninstallApp();
		} else if (requestCode == REQUEST_RESOLUTION) {
...
//				AppFeedback.showAppMenuFeedback(sContext);
	...
	public void onBackPressed() {
		...
		if (TabDataManager.getInstance().getTabStackSize() > 1) {
			...
			// 退出二级页面时候，标识为是按返回键
			if (null != mMainViewGroup) {
				mMainViewGroup.setBackPress(true);
			}
			...
			if (cGroup == null || cGroup.position == 0 || (!cGroup.isIconTab)
				...
				AppsManagementActivity.sendHandler("",
						...
						IAppManagerMsgId.SKIP_SIDE_TAB, 0, new int[] { 0, -1 }, null);
			}
		} else {
			// 没有tab，直接退出
			super.onBackPressed();
