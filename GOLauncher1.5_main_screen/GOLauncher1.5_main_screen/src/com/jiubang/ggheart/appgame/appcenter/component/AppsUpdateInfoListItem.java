	...
	public void onClick(View v) {
		...
		if (mAppBean != null) {
			AppsManagementActivity.sendMessage(this,
					...
					IAppManagerMsgId.APPS_MANAGEMENT_OPERATION_BUTTON, 2, mAppBean,
					null);
			...
			case AppBean.STATUS_DOWNLOAD_FAILED:
				...
				AppManagementStatisticsUtil.getInstance().saveUpdataClick(
						getContext(), mAppBean.mPkgName, mAppBean.mAppId, 1, mAppBean.mICallBackUrl);
				AppsManagementActivity.getApplicationManager().actionDownload(
						getContext(), this);
				RealTimeStatisticsUtil.upLoadDownloadStaticData(getContext(),
						RealTimeStatisticsContants.FUN_ID_APPGAME,
						String.valueOf(mAppBean.mAppId),
						RealTimeStatisticsContants.OPERATE_UPDATE_CLICK,
						RealTimeStatisticsContants.UPDATE_CATEGORY_ID, mPositon + 1,
						mAppBean.mPkgName);
			...
			case AppBean.STATUS_DOWNLOAD_COMPLETED:
				...
				AppsManagementActivity.sendHandler(this,
						...
						IAppManagerMsgId.APPS_MANAGEMENT_INSTALL_APP, 0,
	...
	private void doOnItemClick(AppBean appBean) {
		...
		if (urlHashMap != null && urlHashMap.size() > 0) {
			...
			if (detailUrl != null && !"".equals(detailUrl)) {
				...
				AppManagementStatisticsUtil.getInstance().saveUpdataClick(
						getContext(), appBean.mPkgName, appBean.mAppId, 0, appBean.mICallBackUrl);
...
//							getContext(), appBean.mPkgName, appBean.mAppId, 1);
					...
					if (detailUrl != null && !"".equals(detailUrl)) {
						...
						Toast.makeText(getContext(),
								R.string.themestore_url_fail, Toast.LENGTH_SHORT)
								.show();
					}
				}
				...
				// 统计：国外---保存点击更新统计(记作点击过更新)
				AppManagementStatisticsUtil.getInstance().saveUpdataClick(
						getContext(), appBean.mPkgName, appBean.mAppId, 1, appBean.mICallBackUrl);
				//add by zzf 实时统计
				RealTimeStatisticsUtil.upLoadDownloadStaticData(getContext(),
						RealTimeStatisticsContants.FUN_ID_APPGAME,
