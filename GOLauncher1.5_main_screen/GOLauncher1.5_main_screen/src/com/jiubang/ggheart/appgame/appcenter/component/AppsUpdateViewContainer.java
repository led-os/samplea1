...
public class AppsUpdateViewContainer extends LinearLayout implements
	...
	public void setmControlInfo(byte appIconControl, byte gostoreControl,
		...
		// 功能表Menu菜单
		preferences.putInt(AppGameConstants.APPFUNC_APPMENU_SHOW_MESSAGE,
				appFuncMenuControl);
		preferences.commit();
	}
	...
	private void sendRequest() {
		...
		AppsManagementActivity.sendMessage(this,
				...
				IAppManagerMsgId.REFRESH_UPDATE_DATA, 0, null, null);
	}
	...
	public boolean handleMessage(Object who, int msgId, int param, Object ...obj) {
		switch (msgId) {
		case IAppManagerMsgId.TOP_OF_LISTVIEW:
			if (mUpdateListView != null && param >= 0) {
				mUpdateListView.setSelectionFromTop(param, 0);
			}
			break;
		case IAppManagerMsgId.CHANGE_APPLIST_INFO:
			...
			break;
		case IAppManagerMsgId.SEND_APP_TO_UPDATE_VIEW:
			if (obj[0] != null) {
				if (param == 1) {
					AppBean appBean = (AppBean) obj[0];
					...
					ArrayList<AppBean> appBeanList = (ArrayList<AppBean>) obj[0];
		...
		case IAppManagerMsgId.APPS_MANAGEMENT_OPERATION_BUTTON:
			if (mOperationButton != null) {
				if (param == 1) {
					...
					AppBean removeBean = (AppBean) obj[0];
