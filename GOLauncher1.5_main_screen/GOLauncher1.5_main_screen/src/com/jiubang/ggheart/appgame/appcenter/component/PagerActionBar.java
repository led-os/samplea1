...
public abstract class PagerActionBar extends View {
		...
		if (position >= 0 && position < titleCount) {
			...
			if (!mScrolling) {
				if (position == mCurrentPage) {
					...
					left = leftEdge - mPadding;
				} else if (position == mCurrentPage + 1) {
					left = rightEdge - textWidth + mPadding;
				} else {
				...
				if (mScrollToLeft) {
					...
					if (position < mPageScrollTo) {
						if (position == mPageScrollTo - 1) {
							left = leftOfCenterItem;
							scrollDistance = Math.abs(leftOfCenterItem + mPadding - leftEdge);
						} else {
							left = leftEdge - mPadding;
						...
						scrollDistance = textWidth - mPadding + screenWidth / 2.0f;
					}
				} else {
					...
					if (position > mPageScrollTo) {
						if (position == mPageScrollTo + 1) {
							left = leftOfCenterItem;
							scrollDistance = Math.abs(rightEdge + mPadding - rightOfCenterItem);
						} else {
							left = rightEdge - textWidth + mPadding;
							scrollDistance = screenWidth;
						}
						...
						left = leftEdge - mPadding;
						scrollDistance = Math.abs(leftEdge - leftOfCenterItem - mPadding);
					} else {
	...
	// if (bgBit == null || isPortait != GoLauncherActivityProxy.isPortait()) {
	// createBitmap();
	// isPortait = GoLauncherActivityProxy.isPortait();
	// }
	// if (bgBit != null) {
