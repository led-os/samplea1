...
import com.jiubang.ggheart.appgame.base.component.GoStoreDetailScrollViewGroup.ViewGroupListener;
...
public class AppsDetailContainer {
	...
	private void initAdView() {
		try {
			...
			mAdView = (RelativeLayout) mView.findViewById(R.id.bottom_adview);
...
//				AdConstanst.POS_ID_LAUNCHER_STORE_DETAIL_BANNER, true);
	...
	private boolean getAdmobFullView() {
		mInterstitialAd = AdViewBuilder.getBuilder().getInterstitialAd((Activity) mContext, AdConstanst.PRODUCT_ID_LAUNCHER_STORE, 
				AdConstanst.POS_ID_LAUNCHER_STORE_DETAIL_FULLSCREEN);
			...
			public void onClick(View v) {
				...
				if (mStartType == AppsDetail.START_TYPE_WIDGET_APP) {
					...
					AppsManagementActivity.startAppCenter(mContext,
							...
							RealTimeStatisticsContants.AppgameEntrance.WIDGET);
				}
			}
		});
	...
	private void initViewByBean(final AppDetailInfoBean infoBean) {
		...
		if (mView != null) {
			...
			if (thumbnailUrlsArrayList != null
				...
				mAppDetailIndicators.setIndicatorCount(size + 1);
				mGoStoreDetailScrollViewGroup
						.setIndicators(mAppDetailIndicators);
				ImageView simpleImageView = null;
				// flag为true,加载截图
				boolean flag = true;
				if (GoStorePhoneStateUtil.isWifiEnable(ApplicationProxy.getContext())
						...
						simpleImageView
								.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
										...
										// 添加当前图片索引值
										intent.putExtra(
												GoStorePublicDefine.SCAN_IMAGE_AD_ID_KEY,
												AdConstanst.POS_ID_LAUNCHER_STORE_APP_DETAIL_FULLSCREEN);
										mContext.startActivity(intent);
									}
								});
					} else {
		...
		mGoStoreDetailScrollViewGroup.addView(image);
...
//			.setVisibility(View.VISIBLE);
	...
	private void initScrollFinish() {
		if (mGoStoreDetailScrollViewGroup != null) {
			mGoStoreDetailScrollViewGroup.setScrollFinish(new ViewGroupListener() {
				
				@Override
				public void scrollFinish(int page) {
					// TODO Auto-generated method stub
					int childCount = mGoStoreDetailScrollViewGroup.getChildCount();
					mIsLastScreenShowing = false;
					if (childCount == page + 1) {
