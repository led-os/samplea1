...
public class AppGameInstalledFilter {
	 ...
	 * 过滤已经安装应用,全部过滤
	 */
	public static List<BoutiqueApp> filterAppListAll(
		...
		for (BoutiqueApp app : src) {
			boolean isInstall = AppGameInstallingValidator.getInstance().isAppExist(
					ApplicationProxy.getContext(), app.info.packname);
			if (!isInstall) {
				ret.add(app);
			} 
		}
	...
	public static List<BoutiqueApp> filterAppList(int datatype,
		...
		for (BoutiqueApp app : src) {
			if (remain <= 0) {
				...
				// 精品推荐banner图不会过滤
				ret.add(app);
			} else if (datatype == ClassificationDataBean.ONEPERLINE_SPECIALSUBJECT_TYPE) {
				boolean isInstall = AppGameInstallingValidator.getInstance().isAppExist(
						ApplicationProxy.getContext(), app.info.packname);
				boolean isCanUpdate = false;
				if (isInstall) {
					isCanUpdate = AppGameInstallingValidator.getInstance().isCanUpdate(ApplicationProxy.getContext(), 
