...
public class DownloadUtil {
	...
	public static String getAppCenterHost(Context context) {
		String requesUrl = AppsNetConstant.sAppCenterUrlChina;
		if (!GoStorePhoneStateUtil.isCnUser(context)) {
			requesUrl = AppsNetConstant.sAppCenterUrlOthers;
		}
		return requesUrl;
	}
	...
	public static String getAlternativeAppCenterHost(Context context) {
		String requesUrl = AppsNetConstant.sAppCenterUrlOthers;
		if (!GoStorePhoneStateUtil.isCnUser(context)) {
			requesUrl = AppsNetConstant.sAppCenterUrlChina;
		}
		return requesUrl;
	}
	...
	public static void sendCBackUrl(final String url) {
		...
		//本地没有网络时，就不发送
		if (!Machine.isNetworkOK(ApplicationProxy.getApplication())) {
			return;
		}
