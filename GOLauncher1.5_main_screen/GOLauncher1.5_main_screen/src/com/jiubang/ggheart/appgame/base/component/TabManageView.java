...
public abstract class TabManageView extends LinearLayout implements ScreenScrollerListener {
	...
	private void initView() {
		...
		AppsManagementActivity.sendHandler(mContext,
				...
				IAppManagerMsgId.SHOW_SEARCH_BUTTON, 0, null, null);
	...
	public void showFakeGridTitleBar() {
		Runnable run = new Runnable() {
			...
			public void run() {
				...
				AppsManagementActivity.sendHandler(mContext,
						IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME,
						IAppManagerMsgId.SHOW_SEARCH_BUTTON, 1, null, null);
				mTitleBar.setVisibility(View.GONE);
	...
	private void updateView(TabDataGroup group, boolean singleActive) {
		if (group == null || mAppGameTabsBar == null || mScrollerViewGroup == null || mTipsView == null) {
		...
		if (scroller != null) {
			...
			currentScreen = scroller.getCurrentScreen();
		}
		// 跳转到首页
		...
		IContainer currentContainer = mContainers.get(position);
		if (currentContainer instanceof GridViewContainer) {
			((GridViewContainer) currentContainer).onVisible(true);
		} else if (currentContainer instanceof EmptyContainer) {
			((EmptyContainer) currentContainer).onVisible(true);
		} else if (currentContainer instanceof NewExtraContainer) {
			((NewExtraContainer) currentContainer).onVisible(true);
		} else if (currentContainer instanceof FeaturedContainer) {
			((FeaturedContainer) currentContainer).onVisible(true);
		}
	...
	private void updateContentSide(TabDataGroup group, int targetIndex, int targetSubIndex) {
		...
		if (mScrollerViewGroup != null && mScrollerViewGroup.getScreenScroller() != null) {
			currentScreen = mScrollerViewGroup.getScreenScroller().getCurrentScreen();
		}
		...
		if (currentScreen == position && group.data != null && group.data.size() > position) {
			...
			if (typeId != -1) {
				...
				AppsManagementActivity.sendHandler(mContext,
						IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME,
						IAppManagerMsgId.SAVE_TAB_CLICK, typeId, null, null);
			}
		}
		// 调整显示的首页面
        ...
        AppsManagementActivity.sendHandler(mContext,
		IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME,
		IAppManagerMsgId.SHOW_SEARCH_BUTTON, 1,
		null, null);
	...
	private void updateContent(TabDataGroup group, int targetIndex, int targetSubIndex) {
		if (group == null || mScrollerViewGroup == null || mTitleBar == null) {
			// group不能为空
		...
		if (group.isIconTab) {
			...
			if (mIsFirstUpdateTab) {
				if (TabDataManager.getInstance().getTabStackSize() == 1) {
					...
					AppsManagementActivity.sendHandler("",
							IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME,
							IAppManagerMsgId.TOPTAB_VIEW_LOAD_FINISH, -1, null, null);
				}
        ...
        AppsManagementActivity.sendHandler(mContext,
		IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME,
		IAppManagerMsgId.SHOW_SEARCH_BUTTON, 0,
		null, null);
		mTitleBar.setVisibility(View.VISIBLE);
		// 调整显示的首页面
	...
	public void onScrollFinish(int currentScreen) {
		try {
			...
			IContainer currentContainer = mContainers.get(currentScreen);
			if (currentContainer instanceof GridViewContainer) {
				((GridViewContainer) currentContainer).onVisible(true);
			} else if (currentContainer instanceof EmptyContainer) {
				((EmptyContainer) currentContainer).onVisible(true);
