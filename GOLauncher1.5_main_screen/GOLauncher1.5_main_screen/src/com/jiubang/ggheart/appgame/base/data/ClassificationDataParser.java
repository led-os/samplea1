...
public class ClassificationDataParser {
	...
	public static Map<Integer, ClassificationDataBean> parseNewSubTabData(Context context,
		...
		try {
			...
			//　搜索界面是否开启使用Quixey的提示
			int show_quixey = result.optInt(MessageListBean.TAG_QUIXEY, 0);
			if (show_quixey == 0) {
				//不开启
				GoMarketPublicUtil.getInstance(context).setIsNeedQuixey(false);
			} else {
				GoMarketPublicUtil.getInstance(context).setIsNeedQuixey(true);
			}
