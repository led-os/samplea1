			...
			if (mContainers != null && mCurrentIndex != tabIndex && tabIndex >= 0
				...
				if (mCurrentIndex >= 0 && mCurrentIndex < mContainers.size()) {
					...
					AppsManagementActivity.sendHandler(getContext(),
							...
							IAppManagerMsgId.SAVE_TAB_CLICK, container.getTypeId(), null, null);
					//通知页面转成激活状态
	...
	public void fillupMultiContainer(List<CategoriesDataBean> cBeans, List<IContainer> containers) {
		...
		if (hasADBanner) {
			...
			LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
					DrawUtils.sWidthPixels * 2, LinearLayout.LayoutParams.WRAP_CONTENT);
		...
		// 统计tab点击数
		AppsManagementActivity.sendHandler(getContext(),
				IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME, IAppManagerMsgId.SAVE_TAB_CLICK, containers
