...
public class TabController {
	...
	private static void showPrevLoadProgress() {
		AppsManagementActivity.sendHandler("", IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME,
				IAppManagerMsgId.SHOW_PREVLOAD_PROGRESS, -1, null, null);
	}
	...
	private static void hidePrevLoadProgress() {
		AppsManagementActivity.sendHandler("", IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME,
				IAppManagerMsgId.HIDE_PREVLOAD_PROGRESS, -1, null, null);
	}
...
//							Log.e("TabController", "getNewSubTabData json = " + json);
							...
							if (map != null) {
								...
								AppsManagementActivity.sendHandler("",
										...
										IAppManagerMsgId.REFRESH_TOPTAB_DATA, -1, list, null);
							}
						} catch (Exception e) {
							e.printStackTrace();
						}
	...
	public static void skipToTheNextTab(int typeId, String title, int access,
		...
		AppsManagementActivity.sendHandler(sTabManageView.getContext(),
				IDiyFrameIds.APPS_MANAGEMENT_MAIN_VIEW_FRAME, IAppManagerMsgId.REMOVE_SEARCH_VIEW, 0,
				null, null);
	...
	private static TabDataGroup initTabDataGroup(int typeId, String title,
		...
		if (bean.dataType == ClassificationDataBean.ICON_TAB_TYPE) {
			...
			for (int i = 0; i < categoryList.size(); i++) {
				...
				if (group.adBean != null
						&& (cbean.dataType == ClassificationDataBean.GRID_SORT || cbean.dataType == ClassificationDataBean.GRID_TYPE)) {
					//九宫格分类和九宫格类型的广告推荐位修改成可以往上推
					cbean.adBanner = group.adBean;
					isUseAdBanner = true;
				}
			...
			if (isUseAdBanner) {
				// 九宫格分类和九宫格类型的广告推荐位修改成可以往上推，把九宫格分类和九宫格类型的广告推荐位置为空
				group.adBean = null;
			}
			return group;
		} else {
