...
public class FeaturedContainer extends FrameLayout implements IContainer, IModeChangeListener {
	...
	private void initAdView() {
		mGOadView = GoStoreAdView.getAdView(
		...
		if (mGOadView != null) {
			...
			mAdView.addView(mGOadView, addParams);
		}
	}
	/**
	 * 加载下一页
	...
	public void updateContent(ClassificationDataBean bean, boolean isPrevLoadRefresh) {
		...
		// TODO
		if (mHeadView != null && mFirstSet) {
			try {
				mHeadView.addView(mSummaryView);
				mHeadView.addView(mAdView);
				mFeatureListView.addHeaderView(mHeadView);
				mFirstSet = false;
			} catch (Exception e) {
